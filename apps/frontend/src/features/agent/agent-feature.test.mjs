...
assert.match(agentTypes, /conversationId: string/);
assert.match(agentTypes, /conversationId\?: string \| null/);
...
assert.match(agentChatWidget, /pilo-agent-conversation/);
assert.match(agentChatWidget, /readPersistedConversation/);
assert.match(agentChatWidget, /persistConversation/);
...
assert.match(
  agentChatWidget,
  /<div className="ml-auto flex items-center gap-1">[\s\S]*aria-label="새 대화"[\s\S]*<SquarePen className="size-4" \/>[\s\S]*aria-label="AI 채팅 닫기"/,
  "chat header actions should keep the new conversation control beside close"
);
assert.match(
  agentChatWidget,
  /className="min-h-11 flex-1 resize-none rounded-lg border border-input bg-background[^"]*text-\[15px\][^"]*placeholder:text-muted-foreground/,
  "chat composer should expose a taller semantic textarea"
);
assert.match(
  agentChatWidget,
  /type="submit"[\s\S]*size="icon-lg"[\s\S]*className="size-11"/,
  "chat submit button should match the 44px textarea height"
);
