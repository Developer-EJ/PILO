...
export function WorkspaceCreationPage() {
  ...
  const [stage, setStage] = useState<Stage>(callback.workspaceId ? callback.step === "repositories" || callback.step === "projects" ? "repositories" : callback.step === "project-oauth" ? "projects" : "installation" : "create");
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        const result = await api.listGithubRepositories(workspaceId, { limit: 100 });
        if (!stopped) { setRepositories(result.data); setStage("repositories"); }
  ...
  async function createAndConnect(connect: boolean) {
    ...
    try {
      const workspace = await createWorkspace(session.accessToken, { name: workspaceName.trim(), icon: null });
      ...
      if (!oauth.connected) {
        const start = await api.startGithubOAuth({ returnUrl: createGithubOnboardingReturnUrl(workspace.id, "oauth") });
        window.location.assign(start.authorizeUrl); return;
      }
      const install = await api.startGithubAppInstallation(workspace.id, { returnUrl: createGithubOnboardingReturnUrl(workspace.id, "installation") });
  ...
  if (stage === "create") return <main className="mx-auto grid min-h-screen max-w-xl place-items-center p-6"><Card className="w-full"><CardHeader><CardTitle>workspace 만들기</CardTitle><CardDescription>workspace를 만든 뒤 GitHub 연결을 이어갑니다.</CardDescription></CardHeader><CardContent className="grid gap-4"><Input value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} placeholder="PILO Product Team" /><Button disabled={busy || !workspaceName.trim()} onClick={() => void createAndConnect(true)}>GitHub 연결 후 계속</Button><Button disabled={busy || !workspaceName.trim()} variant="outline" onClick={() => void createAndConnect(false)}>GitHub 없이 workspace 만들기</Button>{message ? <p className="text-sm text-destructive">{message}</p> : null}</CardContent></Card></main>;
  if (stage === "installation" || stage === "syncing") return <WorkspaceCenteredStatus icon={message ? <TriangleAlert /> : <Loader2 className="animate-spin" />} text={message ?? "GitHub App 설치와 source sync를 기다리는 중입니다. 취소했다면 이 화면에서 다시 연결할 수 있습니다."} action={message && workspaceId ? <Button onClick={() => void createAndConnect(true)}>GitHub 다시 연결</Button> : undefined} />;
