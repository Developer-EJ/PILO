...
def meeting_hybrid_lookup_line(
    report_title: str,
    count: int,
    **output: object,
) -> str:
    payload = {
        ...
        "count": count,
        **output,
        "meetingReportHybridLookup": {
            "requestedReportTitle": report_title,
            "exactMatchCount": count,
            "selector": {"reportTitle": report_title},
        },
    }
    return "tool list_meeting_reports: " + json.dumps(
        payload,
        ensure_ascii=False,
        separators=(",", ":"),
    )
...
def meeting_search_catalog(tools: list[dict[str, object]]) -> dict[str, object]:
    catalog = meeting_hybrid_and_list_catalog(tools)
    examples = [
        {"kind": "canonical", "utterance": "배포 지연을 논의한 회의록 찾아줘"},
        {"kind": "paraphrase", "utterance": "API v2가 언급된 회의 찾아줘"},
        {"kind": "typo", "utterance": "배포 지연 논의한 회의록 찿아줘"},
        {"kind": "honorific", "utterance": "관련 발언이 나온 회의를 찾아주세요"},
        {"kind": "abbreviation", "utterance": "회의 내용 근거 검색"},
    ]
    catalog["capabilities"].append(
        {
            "id": "meeting.evidence.search",
            "domain": "meeting",
            "toolNames": ["search_meeting_transcript"],
            "whenToUse": "제목 없는 회의 내용 근거를 검색할 때",
            "mustNotUseFor": ["명시된 제목 범위 검색", "회의록 목록"],
            "positiveExamples": [example["utterance"] for example in examples],
            "examples": examples,
            "selectorKinds": ["query"],
            "requiresConfirmation": False,
    ...
    for descriptor in catalog["descriptors"]:
        if descriptor["toolName"] == "search_meeting_transcript":
            descriptor["capabilityIds"].append("meeting.evidence.search")
    catalog["sha256"] = compute_tool_capability_catalog_sha(
        catalog["version"], catalog["capabilities"], catalog["descriptors"]
    )
    return catalog
...
def test_hybrid_candidate_selection_resume_does_not_require_title_in_latest_prompt() -> None:
    job = _meeting_transcript_search_job()
    planning_context = (
        'selected meeting candidate resume: {"clarificationToolName":"search_meeting_transcript",'
        '"goalToolName":"","resourceType":"meeting_report",'
...
def test_grounded_answer_does_not_claim_workspace_fallback_when_it_was_not_used() -> None:
    handoff = FakeGroundedAnswerHandoffClient(
        {
            "prompt": "'온보딩 주간회의'에서 배포 일정 찾아줘",
            "retrievalContext": {
                "requestedReportTitle": "온보딩 주간회의",
                "exactTitleMatchFound": False,
                "workspaceFallbackApplied": False,
            },
            "sources": [
                {
                    "citationId": "citation_valid",
                    "sourceType": "meeting_transcript",
                    "excerpt": "요청한 제목의 회의록을 찾지 못했습니다.",
                }
            ],
        }
    )
    processor = FakeGroundedAnswerProcessor(
        ...
        model="unused",
        timeout_seconds=1,
        answers=[("요청한 제목의 회의록을 찾지 못했습니다.", ["citation_valid"])],
    )
    ...
    assert result.reason == "grounded_answer_completed"
    assert "Workspace 전체 회의 내용" not in handoff.completed[0][1]
...
@pytest.mark.parametrize("context_surface", ["sql_erd", "pr_review", "canvas"])
def test_meeting_evidence_capability_is_available_from_every_surface(
    context_surface: str,
) -> None:
    tools = [
        tool_snapshot(
    ...
    assert job.tool_capability_catalog is not None
    request = AgentRoutingRequest(
        prompt="배포 일정이 미뤄진 이유를 논의한 회의록 찾아줘",
        timezone="Asia/Seoul",
    ...
    prompt = json.loads(_agent_router_user_prompt(request))
    capability_ids = {item["id"] for item in prompt["capabilities"]}

    assert "meeting.evidence.search" in capability_ids
    assert "meeting.report.hybrid_search" in capability_ids


def test_content_words_do_not_override_router_selected_report_list() -> None:
    tools = [
    ...
    corrected = _normalize_meeting_report_search_routing(
        ...
        prompt="배포 일정이 미뤄진 이유를 논의했던 회의록을 찾아줘",
    )

    assert corrected.capability_ids == ("meeting.reports.list",)
...
def test_natural_named_meeting_summary_does_not_override_list_routing(
    prompt: str,
    expected_title: str,
) -> None:
    tools = [
...
def test_natural_named_evidence_words_do_not_override_list_routing(
    prompt: str,
    expected_title: str,
) -> None:
    tools = [
...
def test_natural_meeting_evidence_words_do_not_override_list_routing(
    prompt: str,
) -> None:
    tools = [
        tool_snapshot(name="list_meeting_reports", inputSchema={"type": "object"}),
...
def test_evidence_words_do_not_override_router_selected_summary() -> None:
    tools = [
        tool_snapshot(name="list_meeting_reports", inputSchema={"type": "object"}),
        tool_snapshot(name="search_meeting_transcript", inputSchema={"type": "object"}),
    ]
...
def test_explicit_title_evidence_words_do_not_override_summary_routing(
    prompt: str,
) -> None:
    tools = [
        tool_snapshot(name="list_meeting_reports", inputSchema={"type": "object"}),
...
def test_general_decision_words_do_not_override_direct_evidence_routing(
    prompt: str,
...
def test_processor_respects_report_list_routing_from_other_surface() -> None:
    tools = [
        tool_snapshot(
            name="list_meeting_reports",
            inputSchema={"type": "object", "properties": {}},
    ...
    assert result.reason == "agent_execution_handoff_completed"
    assert len(router.requests) == 1
    assert [tool.name for tool in planner.requests[0].tools] == ["list_meeting_reports"]
    assert planner.requests[0].routing.capability_ids == ("meeting.reports.list",)
...
def test_processor_builds_title_scope_for_router_selected_summary(
    prompt: str,
    expected_title: str,
) -> None:
    tools = [
...
def test_processor_replaces_prior_named_summary_scope_with_current_turn_title() -> None:
    ...
    planner = FakePlannerClient(
        decision=planner_decision(
            tool_name="summarize_meeting_report",
            tool_input={"reportTitle": "금요일 데일리 스크럼"},
        )
    )
    router = FakeRouterClient(
        decision=routing_decision(
...
def test_explicit_title_discussion_request_corrects_direct_search_to_hybrid() -> None:
    tools = [
        tool_snapshot(name="list_meeting_reports", inputSchema={"type": "object"}),
        tool_snapshot(name="search_meeting_transcript", inputSchema={"type": "object"}),
    ]
    ...
    corrected = _normalize_meeting_report_search_routing(
        AgentRoutingDecision(
            ...
            domains=("meeting",),
            capability_ids=("meeting.evidence.search",),
            intent_summary="회의 내용 검색",
            confidence="high",
            clarification_question=None,
            unsupported_reason=None,
        ),
        job.tool_capability_catalog,
        prompt="‘온보딩 주간회의’에서 배포 일정 논의를 찾아줘",
    )

    assert corrected.capability_ids == ("meeting.report.hybrid_search",)


def test_completed_planner_routing_continues_without_rerouting() -> None:
    tools = [
        tool_snapshot(name="list_meeting_reports", inputSchema={"type": "object"}),
        tool_snapshot(name="search_meeting_transcript", inputSchema={"type": "object"}),
    ]
    ...
    routing = _continued_agent_routing(
        ...
        "\n".join(
            [
                'routed workflow: {"capabilityIds":["meeting.report.hybrid_search"]}',
                meeting_hybrid_lookup_line("온보딩 주간회의", 1, reports=[]),
            ]
        ),
    )

    assert routing is not None
    assert routing.capability_ids == ("meeting.report.hybrid_search",)
...
def test_normalizer_routes_calendar_detail_ordinal_to_calendar_context_reference() -> None:
        ...
        agent_payload(
            tools=[
                ...
                tool_snapshot(
                    ...
                    description="MeetingReport 상세 조회",
                    executionMode="contextual",
                    inputSchema={"type": "object", "properties": {}},
                ),
            ]
    ...
    normalized = normalize_agent_planner_decision(
        planner_decision(
            ...
            tool_input={"contextRef": third_context_ref},
        ),
        job,
        prompt="세 번째 일정에 대해서 자세히 알려줘",
        planning_context=(
            'previous resource: {"turn":2,"contextRef":"ctx_0123456789abcdef01234567",'
            '"resourceType":"event","ordinal":1}\n'
            'previous resource: {"turn":2,"contextRef":"ctx_89abcdef0123456701234567",'
...
def test_normalizer_rejects_calendar_detail_ordinal_outside_latest_result_set() -> None:
    ...
    normalized = normalize_agent_planner_decision(
        planner_decision(tool_name="get_calendar_event", tool_input={}),
        job,
        prompt="세 번째 일정 자세히 알려줘",
        planning_context=(
            'previous resource: {"turn":2,"contextRef":"ctx_0123456789abcdef01234567",'
            '"resourceType":"event","ordinal":1}\n'
            'previous resource: {"turn":2,"contextRef":"ctx_89abcdef0123456701234567",'
    ...
    assert normalized.status == "needs_clarification"
    assert normalized.output_summary["missingFields"] == ["calendar_event_context"]
...
def test_normalizer_keeps_latest_meeting_report_list_candidate() -> None:
    ...
    normalized = normalize_agent_planner_decision(
        ...
        job,
        prompt="최신 회의록 1건 보여줘",
    )

    assert normalized.status == "tool_candidate"
    assert normalized.output_summary["input"] == {"latest": True, "limit": 1}


@pytest.mark.parametrize(
    ("prompt", "current_date", "expected_input"),
    [
        ("최근 회의록 보여줘", "2026-07-15", {"latest": True}),
...
def test_normalizer_enforces_latest_one_for_unqualified_meeting_report_list() -> None:
    ...
    assert normalized.status == "tool_candidate"
    assert normalized.output_summary["input"] == {"latest": True}
...
def _meeting_unified_search_scope_job() -> AgentRunJob:
    return parse_agent_run_job_payload(
        agent_payload(
            tools=[
                tool_snapshot(
                    name="search_meeting_reports",
                    description="MeetingReport 통합 검색",
                    executionMode="contextual",
                    inputSchema={
                        "type": "object",
                        "required": ["query"],
...
def test_current_turn_search_scope_drops_stale_selectors_and_keeps_current_date() -> None:
    normalized = normalize_agent_planner_decision(
        planner_decision(
            tool_name="search_meeting_reports",
            tool_input={
...
def test_latest_completed_scope_keeps_status_and_latest_together() -> None:
    ...
    normalized = normalize_agent_planner_decision(
        planner_decision(
            tool_name="list_meeting_reports",
            tool_input={"reportTitle": "이전 회의", "status": "FAILED"},
        ),
        job,
        prompt="완료된 가장 최신 회의록 알려줘",
        current_date="2026-07-28",
        timezone="Asia/Seoul",
    )
...
def test_workspace_fallback_requires_explicit_current_turn_request() -> None:
    normalized = normalize_agent_planner_decision(
        planner_decision(
            tool_name="search_meeting_reports",
            tool_input={
...
def test_explicit_scope_continuation_preserves_previous_search_filters() -> None:
    normalized = normalize_agent_planner_decision(
        planner_decision(
            tool_name="search_meeting_reports",
            tool_input={
        ...
        _meeting_unified_search_scope_job(),
        prompt="같은 검색 조건을 그대로 유지해서 회의록 다시 찾아줘",
        current_date="2026-07-28",
        timezone="Asia/Seoul",
        routed_capability_ids=("meeting.report.unified_search",),
    )

    scope = normalized.output_summary["input"]
    assert scope["fallback"] == "none"
    assert scope["from"] == "2026-07-16T15:00:00.000Z"
    assert scope["reportTitle"] == "온보딩 회의"
    assert scope["roomName"] == "Backend"
    assert scope["status"] == "COMPLETED"
    assert scope["to"] == "2026-07-17T15:00:00.000Z"
...
def test_hybrid_title_lookup_recovers_explicit_quoted_title_after_other_normalizers() -> None:
    job = parse_agent_run_job_payload(
        agent_payload(
            tools=[
                tool_snapshot(
...
def test_hybrid_title_lookup_without_explicit_or_planner_title_never_lists_latest() -> None:
    ...
    normalized = normalize_agent_planner_decision(
        planner_decision(tool_name="list_meeting_reports", tool_input={}),
        job,
        prompt="회의록에서 배포 일정 논의를 찾아줘",
        current_date="2026-07-23",
        completion_tool_names=("search_meeting_transcript",),
        routed_capability_ids=("meeting.report.hybrid_search",),
    )
    ...
    assert normalized.status == "needs_clarification"
    assert "toolName" not in normalized.output_summary
    assert normalized.output_summary["missingFields"] == ["meeting_report_title"]
...
def test_hybrid_search_exact_one_uses_current_run_opaque_context_ref() -> None:
    ...
    normalized = normalize_agent_planner_decision(
        ...
        planning_context=(
            ...
            + meeting_hybrid_lookup_line(
                "온보딩 주간회의",
                1,
                reports=[{"title": "온보딩 주간회의"}],
            )
            + "\n"
            'previous resource: {"turn":2,"contextRef":"'
            + context_ref
...
def test_hybrid_search_exact_zero_falls_back_to_workspace_content_query() -> None:
    normalized = normalize_agent_planner_decision(
        ...
        planning_context=(
            "user: ‘온보딩 주간회의’에서 API 배포 일정을 어떻게 정했어?\n"
            + meeting_hybrid_lookup_line("온보딩 주간회의", 0, reports=[])
        ),
        routed_capability_ids=("meeting.report.hybrid_search",),
    )
...
def test_unrelated_zero_count_list_is_not_hybrid_workspace_fallback() -> None:
    normalized = normalize_agent_planner_decision(
        ...
        planning_context=(
            "user: ‘온보딩 주간회의’에서 API 배포 일정을 어떻게 정했어?\n"
            'tool list_meeting_reports: {"reportTitle":"무관한 회의",'
            '"count":0,"reports":[]}'
        ),
        routed_capability_ids=("meeting.report.hybrid_search",),
    )

    assert "meetingReportHybridContext" not in normalized.output_summary
    assert normalized.output_summary["input"]["reportTitle"] == "온보딩 주간회의"
...
def test_hybrid_workspace_fallback_removes_title_date_and_command_wording() -> None:
    normalized = normalize_agent_planner_decision(
        ...
        _meeting_transcript_search_job(),
        prompt=(
            "7월 18일 ‘온보딩 주간회의’에서 API 배포 일정 찾아줘. "
            "제목이 없으면 Workspace 전체 회의록에서 검색해줘"
        ),
        planning_context=(
...
def test_content_only_search_does_not_invent_a_report_title_lookup() -> None:
    normalized = normalize_agent_planner_decision(
        ...
        planning_context="",
        completion_tool_names=("search_meeting_transcript",),
        routed_capability_ids=("meeting.evidence.search",),
    )
