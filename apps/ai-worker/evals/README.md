...
보고서의 `routingFunnel`은 `routerRouted → domainExact → toolExact → requiredInputExact →
...
기록된다. workflow는 준비한 registry/catalog artifact를 공유하고 baseline/candidate × 4개 variant를 8개
...
`Evaluate Agent Router and Planner` workflow가 고정 Meeting canonical·held-out·counterexample·context로
