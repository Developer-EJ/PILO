...
export class JuhyungTaskService {
  ...
  async createTask(
    ...
    bodyOrActor?: CreateTaskBody | CurrentActor,
    actor?: CurrentActor,
  ): Promise<TaskRecord | TaskSummary> {
    if (typeof workspaceIdOrInput !== "string") {
      return this.createTaskRecord(
        workspaceIdOrInput,
        bodyOrActor as CurrentActor | undefined,
      );
    }

    const workspaceId = workspaceIdOrInput;
    const body = bodyOrActor as CreateTaskBody;
