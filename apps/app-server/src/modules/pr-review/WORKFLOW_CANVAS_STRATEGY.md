...
Post-MVP 상세 계획은 `POST_MVP.md`와
`POST_MVP_COLLABORATIVE_REVIEW_CANVAS_CHECKLIST.md`를 기준으로 한다. 이 문서에서는
MVP에서 Post-MVP로 확장할 때 유지해야 할 경계만 요약한다.

Post-MVP에서는 같은 PR을 위한 공유 리뷰 공간과 `board_type = 'review'` Canvas를 하나씩
만든다. head SHA가 바뀌어도 새 Canvas를 만들지 않고, 같은 공간에 새 리뷰 버전을 추가한다.
PR Review workflow의 의미 데이터는 계속 PR Review가 소유하고 Canvas는 custom shape의
위치·크기·그룹과 사용자 annotation을 소유한다.
...
PR Review workflow model
        |
        v
PR별 공유 Review Room / head SHA별 Revision
        |
        v
workflow-to-canvas-shapes materializer
        |
        v
board_type=review Canvas / custom shapes / realtime
```

확정된 Post-MVP 방향:

- `pr_review_rooms`가 PR과 Review Canvas를 1:1로 연결한다.
- 기존 `pr_review_sessions`는 같은 room 안의 head SHA별 리뷰 버전으로 사용한다.
- `pr_review_file_node`와 `pr_review_relation_edge`는 Canvas에 저장되는 전용 custom
  shape다.
- PR Review가 shape의 도메인 참조와 relation endpoint를 소유한다.
- 사용자는 file node의 위치·크기·그룹, annotation과 사용자 edge를 편집한다.
- 새 버전 materialization은 기존 node geometry와 annotation을 보존한다.
- PR merge/close 후 room은 read-only가 되며 Sidebar의 `리뷰 공간`에서 조회한다.
- Workspace 구성원이 room을 삭제하면 연결된 PR Review와 Canvas 데이터를 영구 삭제한다.
...
- MVP shape ID와 Post-MVP room file identity 사이의 migration을 명시한다.
- Canvas DB persistence와 review board operation 권한은 Canvas owner review가 필요한 별도
  구현 이슈로 다룬다.
