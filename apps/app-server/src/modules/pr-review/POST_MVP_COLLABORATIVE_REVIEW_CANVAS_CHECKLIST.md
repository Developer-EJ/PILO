...
- `pr_review_file_node`
  - 안정적인 ID: `pr-review-file:{roomFileId}`
...
- [ ] `pr_review_rooms`와 room file identity migration 설계
- [ ] 기존 session을 revision으로 연결하는 migration과 backfill 정책 확정
- [ ] room/current revision 상태와 제안 API 계약 반영
- [ ] 동시 room/버전 생성 unique constraint 검증

### 4-B 공유 Room 생성·조회·삭제 Backend

- [ ] idempotent 시작/합류 서비스 구현
- [ ] room 목록과 상세 API 구현
- [ ] 모든 Workspace 구성원 권한 검증
- [ ] 영구 삭제 transaction과 cascade 검증

### 4-C Review Canvas와 Custom Shape 기반

- [ ] `board_type=review` 생성·조회·operation 접근 허용
- [ ] `pr_review_file_node`, `pr_review_relation_edge` 등록
- [ ] 시스템 shape 필드별 수정 권한 검증
- [ ] Canvas 담당자 review 반영

### 4-D 새 버전 분석과 Shape Materialization

- [ ] 같은 room의 head SHA별 session/job 생성
- [ ] 성공 전 기존 current session 유지
- [ ] stable room file identity와 decision carry-over 구현
- [ ] Conflict apply 후 immutable head를 유지하며 successor revision 자동 생성
- [ ] node geometry 보존과 새 node 초기 layout 구현
- [ ] 성공 시 current session 원자 교체
...
- [ ] 새 분석 성공 후 변경되지 않은 node 위치와 판단이 유지된다.
- [ ] 변경된 파일만 `재검토 필요`가 되고 새 파일 node만 초기 배치된다.
- [ ] Conflict apply commit이 기존 revision head를 바꾸지 않고 같은 room의 successor가 된다.
- [ ] materialization 실패 시 current version이 바뀌지 않는다.
