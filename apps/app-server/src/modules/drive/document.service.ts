...
const DRIVE_ITEM_NAME_UNIQUE_CONSTRAINT = "ux_drive_items_workspace_parent_name_active";
const UNIQUE_VIOLATION_CODE = "23505";
const MAX_DEFAULT_NAME_RETRIES = 20;
...
export class DocumentService {
  ...
  async createDocument(
    ...
    for (let attempt = 0; attempt < MAX_DEFAULT_NAME_RETRIES; attempt += 1) {
      try {
        return await this.database.transaction(async (transaction) => {
          const item = await transaction.queryOne<DriveItemRow>(
            ...
            `,
            [documentId, workspaceId, input.parentId, name, currentUserId]
          );
          ...
          await this.activityLogService.append(
            transaction,
            this.buildCreatedActivityLog(currentUserId, workspaceId, documentId, name, input.parentId)
          );
        ...
        if (!this.isDriveItemNameUniqueViolation(error)) throw error;
        if (!input.usesDefaultName) {
          throw badRequest("Drive item name already exists in this folder");
        }

        name = await this.findNextDefaultDocumentName(workspaceId, input.parentId);
      }
    }

    throw conflict("Document name could not be allocated. Please retry.");
  }
  ...
  private async findNextDefaultDocumentName(
    workspaceId: string,
    ...
    const rows = await this.database.query<{ name: string }>(
      ...
      `,
      [workspaceId, parentId, `${DEFAULT_DOCUMENT_NAME}%`]
    );
    const usedIndexes = new Set<number>();

    for (const row of rows) {
      if (row.name === DEFAULT_DOCUMENT_NAME) {
        usedIndexes.add(1);
        continue;
      }

      const match = new RegExp(`^${escapeRegularExpression(DEFAULT_DOCUMENT_NAME)} \\((\\d+)\\)$`).exec(
        row.name
      );
      if (!match) continue;
    ...
    return index === 1 ? DEFAULT_DOCUMENT_NAME : `${DEFAULT_DOCUMENT_NAME} (${index})`;
  }

  private isDriveItemNameUniqueViolation(error: unknown): boolean {
    return (
      typeof error === "object" &&
      error !== null &&
      "code" in error &&
      ...
      "constraint" in error &&
      error.constraint === DRIVE_ITEM_NAME_UNIQUE_CONSTRAINT
    );
  }
