...
const CANVAS_AGENT_TOOL_SHORTCUT_HINTS: Record<string, string> = {
  선택: "단축키는 V야.",
  메모: "단축키는 N이야.",
  프레임: "단축키는 F야.",
...
function withShortcutDescription(target: CanvasAgentToolTarget): CanvasAgentToolTarget {
  const shortcutHint = CANVAS_AGENT_TOOL_SHORTCUT_HINTS[target.label];
...
export const CANVAS_AGENT_TOOL_TARGETS = [
  {
...
function resolveToolTargetByNumber(prompt: string) {
  ...
  return CANVAS_AGENT_TOOL_TARGETS[index] ?? null;
}
...
export function resolveCanvasAgentToolTarget(prompt: string): CanvasAgentToolTargetResolution | null {
  ...
  const match = CANVAS_AGENT_TOOL_TARGETS
    .flatMap((tool) =>
...
export function findCanvasAgentToolTarget(target: string): CanvasAgentToolTarget | null {
  return CANVAS_AGENT_TOOL_TARGETS.find((tool) => tool.target === target) ?? null;
}
