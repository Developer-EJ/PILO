...
assert.match(jobs, /status = 'failed'\s+AND error_message = 'GitHub webhook could not be enqueued'/);
assert.match(jobs, /status = 'processing'\s+AND lease_expires_at < now\(\)/);
