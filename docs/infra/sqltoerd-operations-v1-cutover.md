...
- **source lock:** A가 Source를 열거나 FK source 위치로 이동할 때는 lock 요청이 없어야 한다. A가 SQL editor를 클릭하면 lock을 얻고 Network 기록에서 약 10초 간격 renew를 확인한다. B의 SQL editor와 dialect는 read-only이고 B의 직접 publish 요청도 `409 CONFLICT`여야 한다. A가 source를 변경하지 않은 채 canvas를 클릭하면 release 뒤 B가 재획득해야 한다. A에게 dirty source 또는 pending publish가 있으면 publish 완료 전에는 release하지 않고, 완료 뒤 canvas에 focus가 있으면 release해야 한다. A의 연결을 release 없이 끊은 경우에는 30초 TTL 만료 뒤 B가 획득해야 한다. Source publish의 lease `409` 재시도 중에도 두 browser의 canvas `layout_patch` 저장은 계속되어야 한다.
