...
| `NEXT_PUBLIC_PILO_REALTIME_SERVER_URL` | 브라우저에서 Socket.IO 및 `tldraw_sync` Canvas WebSocket(`/sync/canvas`)으로 연결할 Realtime Server origin. local 개발 기본값은 `http://localhost:3001`이며 production에서는 설정하지 않으면 Canvas presence와 tldraw sync room 연결을 비활성화한다. |
