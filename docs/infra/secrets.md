...
| `NEXT_PUBLIC_PILO_REALTIME_SERVER_URL` | 브라우저에서 Socket.IO, `tldraw_sync` Canvas WebSocket(`/sync/canvas`), 문서 Hocuspocus WebSocket(`/sync/documents`)으로 연결할 Realtime Server origin. `http(s)` 또는 `ws(s)` origin을 설정할 수 있으며 문서 client는 protocol을 WebSocket으로 바꿔 `/sync/documents`를 붙인다. local 개발 기본값은 `http://localhost:3001`이며 production에서는 설정하지 않으면 Canvas presence, tldraw sync, 문서 공동 편집 연결을 비활성화한다. |
