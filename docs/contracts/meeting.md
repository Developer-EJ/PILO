...
| `POST` | `/api/meeting-action-items/:actionItemId/task-draft` | Task draft 생성 요청. Action Item을 converted 처리하지 않음 |
...
`taskDraft.taskId`는 draft adapter 응답 reference다. 실제 Task id가 아니다.
`actionItem.status`와 `convertedTaskId`는 이 endpoint에서 바뀌지 않는다.
...
- `actions`에 `task.create.draft`가 포함될 수 있지만, report output 저장만으로 `meeting_action_items.convertedTaskId`를 채우지 않는다.
...
`MeetingActionItem`은 Task 원본이 아니라 Task 후보이다. `requestActionItemTaskDraft()`는
approved action item을 `TaskCreateDraft` payload로 바꾼 뒤
`TaskDraftClient.createTaskDraft()`를 호출한다. 이 호출이 성공해도 Meeting은 action item의
`status`와 `convertedTaskId`를 바꾸지 않는다. `convertedTaskId`는 실제 Task owner 또는
Agent action executor가 진짜 Task id를 반환한 뒤 `ConvertMeetingActionItemRequest`로만 채운다.
draft 단계 연결 id가 필요하면 `convertedTaskId`를 재사용하지 말고 별도 draft link field를 contract에
추가한다.
...
- Task draft 생성과 승인/거절은 주형 API 또는 세인 Agent action adapter를 통해 수행한다.
...
Task draft adapter를 연결하지 않는 fixture-only 환경에서는 Action Item을 `draft` 또는
`approved`까지만 처리한다.
현재 mock `TaskDraftClient`를 사용하면 `createTaskDraft()` 성공 후에도 Action Item은
`approved`와 `convertedTaskId = null`을 유지한다. mock `taskDraft.taskId`는 draft adapter
응답 reference일 뿐 실제 `tasks` row id가 아니다.
