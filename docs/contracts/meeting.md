...
`TaskDraftClient.createTaskDraft()`를 호출한다. 이 호출이 성공하면 Meeting은 action item을
`converted`로 바꾸고 `convertedTaskId = taskDraft.taskId`를 저장한다. 이 값은 현재 runtime에서
Task owner/Agent action이 반환한 draft/task reference이며, Meeting이 `tasks` table을 직접 쓴다는
뜻이 아니다.
...
`approved`까지만 처리한다.
현재 mock `TaskDraftClient`를 사용하면 `createTaskDraft()` 성공 직후 Action Item은
`converted`가 되고 `convertedTaskId`에는 mock `taskDraft.taskId`가 들어간다. mock은 실제
`tasks` row를 만들지 않는다.
