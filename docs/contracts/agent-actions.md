...
현재 `temp-dev`에는 Agent run/action HTTP controller가 없다.
...
이 skeleton은 app-server module 안에서 직접 호출하는 테스트/후속 구현용 surface이며,
HTTP route, DB-backed persistence, SQS worker integration, owner-domain side effect를
제공하지 않는다.

다른 도메인은 아래 Agent Run API를 현재 runtime에서 호출하면 안 된다.

## Current Internal Skeleton

현재 내부 skeleton은 다음 범위만 구현되어 있다.
...
- action confirmation을 `confirmed`까지 기록.
...
이 항목은 `Current Runtime APIs`가 아니다. controller가 없으므로 외부 소비자는
fixture, mock, 또는 후속 Agent Runtime HTTP 구현 PR을 기다려야 한다.

## Planned Next Runtime Surface

다음 Agent Runtime 구현 PR의 목표 HTTP surface는 아래 네 route로 제한한다.
모두 현재는 Deferred다.
