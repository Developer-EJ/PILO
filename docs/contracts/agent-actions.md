...
| `requiresConfirmation` | 원본 데이터를 생성, 수정, 삭제하는 action은 `true`다. |
| `payload` | 아래 action type별 payload schema 중 하나와 일치해야 한다. |
| `status` | State Machine에 정의된 값만 사용한다. |
| `confirmedByMemberId` | 승인 전에는 `null`, 승인 후에는 `workspace_members.id`다. |
| `confirmedAt` | 승인 전에는 `null`, 승인 시각은 ISO date-time이다. |
...
- `failed`: confirmed 이후 실행 실패 또는 result 처리 실패 terminal 상태다.
