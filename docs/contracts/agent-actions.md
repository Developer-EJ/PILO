...
이후 explicit execute boundary도 Current Mock/In-memory route로 제공한다.

현재 Agent run/action HTTP controller는 in-memory deterministic skeleton을 얇게
노출한다. DB-backed persistence, SQS worker integration, real owner-domain side
effect는 제공하지 않는다.
...
- 현재 execute route는 `task.create.draft`만 mock owner executor로 처리한다.
  Task service/repository/DB를 Agent module에서 직접 호출하지 않는다.
...
- owner domain 실행은 explicit adapter/mock boundary 뒤에 두며, 승인만으로 Task/Meeting/Review/GitHub 원본 데이터를 변경하지 않는다.
- `task.create.draft` 실행은 TaskDraft contract를 검증하고
  `TaskDraftSummary` shape의 mock owner result를 trace metadata에 남긴다.
- 실제 주형 TaskDraft DB/API write adapter 연결은 Deferred follow-up이다.
...
| -------------------------- | ------------ | ------------------------------ | ---------------------------------------------------------------------------------- |
| `task.create.draft`        | 주형         | `TaskCreateDraft`              | Current Mock/In-memory owner executor, returns `TaskDraftSummary`-shaped mock trace |
