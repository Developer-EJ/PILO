...
| Field | Rule |
|---|---|
| `id` | 저장된 `agent_actions.id`다. local runner와 fixture는 deterministic mock id를 사용할 수 있다. |
| `runId` | `agent_runs.id`와 같아야 한다. |
| `type` | `domain.action.target` 형태를 사용한다. |
...
| `status` | State Machine에 정의된 값만 사용한다. `requiresConfirmation = false`인 action은 `waiting_confirmation`, `confirmed`를 건너뛴다. |
...
- `failed`: confirmed 이후 실행 실패, `requiresConfirmation = false` action의 실행 실패, 또는 result 처리 실패 terminal 상태다.
- `requiresConfirmation = false`인 action은 `waiting_confirmation`과 `confirmed`를 건너뛴다.
- 이 경우 `confirmedByMemberId`, `confirmedAt`는 계속 `null`이어야 하며, 상태는 `draft -> executed` 또는 `draft -> failed`로만 진행한다.
