...
아래 네 route만 Current Runtime API로 제공한다.

현재 Agent run/action HTTP controller는 in-memory deterministic skeleton을 얇게
노출한다. DB-backed persistence, SQS worker integration, owner-domain side effect는
제공하지 않는다.
...
| -------------------------- | ------------ | ------------------------------ | --------------------------------------------------- |
| `task.create.draft`        | 주형         | `TaskCreateDraft`              | 현재 Task draft API, returns `TaskDraftSummary`     |
...
The four run/action routes listed in `Current Runtime APIs` are implemented as
