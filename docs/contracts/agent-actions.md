...
```json
{
  ...
  "payload": {
    "workspaceId": "uuid",
    "title": "Create OAuth failure empty state",
...
| `task.assign` | Juhyung | `TaskAssignAction` | Task assignment API |
...
```json
{
  ...
  "status": "succeeded",
  "output": {
    "summary": "OAuth failure handling needs a UI empty state.",
    "decisionCount": 1,
    "actionItemCount": 2,
    "riskCount": 0
  },
...
`AgentJobMessage.input` and `AgentResultMessage.output` are not free-form objects. They are discriminated by `workflowType` and must match `docs/contracts/schemas/pilo-public-contracts.schema.json`.

| workflowType | input schema | output schema |
|---|---|---|
| `meeting.report.generate` | `MeetingReportWorkflowInput` | `MeetingReportWorkflowOutput` |
| `review.analysis.generate` | `ReviewAnalysisWorkflowInput` | `ReviewAnalysisWorkflowOutput` |
| `planning.generate` | `PlanningWorkflowInput` | `PlanningWorkflowOutput` |
| `task.draft.generate` | `TaskDraftWorkflowInput` | `TaskDraftWorkflowOutput` |
| `github.issue.draft.generate` | `GithubIssueDraftWorkflowInput` | `GithubIssueDraftWorkflowOutput` |
| `orchestrator.run` | `OrchestratorRunWorkflowInput` | `OrchestratorRunWorkflowOutput` |

Do not add workflow-specific fields directly to `AgentJobMessage` or `AgentResultMessage`. Add fields to the workflow-owned input/output schema first, then mirror it in TS/Python through a Contract Integration PR.
