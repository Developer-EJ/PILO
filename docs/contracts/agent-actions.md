...
| `status` | State Machine에 정의된 값만 사용한다. `requiresConfirmation = false`인 action은 `waiting_confirmation`, `confirmed`, `failed`를 건너뛴다. |
...
| `planning.approve.draft` | 세인/주형 | `PlanningApproveAction` | 세인 approval, then 주형 Task/Milestone API |
...
- `requiresConfirmation = false`인 action은 `waiting_confirmation`, `confirmed`, `failed`를 건너뛴다.
