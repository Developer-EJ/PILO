...
현재 `temp-dev`에는 Planning HTTP controller가 없다.
아래 API는 모두 Deferred이며, 다른 도메인은 현재 runtime에서 호출하면 안 된다.

## Deferred APIs

| Method | Path | 목적 |
|---|---|---|
| `POST` | `/api/workspaces/:workspaceId/project-plan-drafts` | 계획 초안 생성 |
| `GET` | `/api/project-plan-drafts/:draftId` | 계획 초안 상세 |
| `POST` | `/api/project-plan-drafts/:draftId/recommend-tech-stack` | 기술스택 추천 |
| `POST` | `/api/project-plan-drafts/:draftId/breakdown-features` | 기능 분해 |
| `POST` | `/api/project-plan-drafts/:draftId/assign-roles` | 역할 배정 초안 |
| `POST` | `/api/project-plan-drafts/:draftId/approve` | 계획 승인 |
...
`GET /api/project-plan-drafts/:draftId`의 응답 read model이다. 온보딩 결과 화면은 이 응답 하나로 기술스택, 기능 초안, 역할 초안, 마일스톤 초안, 리스크, 첫 회의 agenda 초안, 승인 결과를 표시한다.
