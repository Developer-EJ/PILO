...
`goal` is nullable until the onboarding answer is captured. `status` uses `ProjectPlanDraftStatus`: `draft`, `reviewing`, `approved`, `rejected`, `waiting_confirmation`, or `approved_pending_task_creation`.
...
- `goal` is nullable until the onboarding answer is captured.
- `ProjectPlanDraftStatus` values are `draft`, `reviewing`, `approved`, `rejected`, `waiting_confirmation`, and `approved_pending_task_creation`.

- `status`는 planning draft 자체의 상태다. 허용 값은 `draft`, `reviewing`, `approved`, `rejected`, `waiting_confirmation`, `approved_pending_task_creation`이다.
