...
- Contract 변경 후 기존 작업자는 `temp-dev`를 최신화한 뒤 구현을 계속한다.
- Breaking change는 바로 제거하지 않고 deprecated 기간을 둔다.
- MD는 설명용이고, 가능한 경우 schema 또는 generated type으로 검증 가능하게 만든다.
- shared/common 파일 변경은 기능 PR에 섞지 않는다. contract/spec/infra/hygiene 성격이 명확한 별도 PR로 분리하고 영향 owner를 적는다.
- mock, fixture, in-memory adapter, Deferred API에 의존하는 PR/Issue는 해당 상태와 실제 연동 후속 Issue를 본문에 명시한다.
...
3. 관련 owner 리뷰를 받는다.
4. `temp-dev`에 contract PR을 먼저 merge한다.
5. 구현 브랜치는 `temp-dev`를 최신화한다.
