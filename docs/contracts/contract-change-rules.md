...
| Machine-readable contract | CI가 검증할 수 있는 계약 | `docs/contracts/openapi/pilo-public-api.yaml`, JSON Schema, generated types |
...
- HTTP API path, method, request body, response body가 바뀌면 `docs/contracts/openapi/pilo-public-api.yaml`을 함께 수정한다.

## Feature PR Central File Restriction

Feature PRs must not directly edit central files. They may edit only their owner-local fragments, shards, and implementation fill points. Central files are changed only by a contract integration PR after owner-local fragments are reviewed.

This does not block all PRs. Use the right PR type:

- Feature PR: no central files changed; merge can proceed after normal review and validation.
- Contract Integration PR: central files changed; the PR must list the owner-local fragments or shards being serialized.
- If a feature PR accidentally changes central files, split those changes out or reclassify the PR as a Contract Integration PR before review.
