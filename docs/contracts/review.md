...
- Review creates and owns `changed_files` and `changed_functions` from `PullRequestChangedFileSummary`; it does not own GitHub PR source sync.
...
주형의 PR sync가 없으면 `PullRequestSummary`와 `PullRequestChangedFileSummary` fixture로 리뷰 화면, changed file/function 저장 흐름, 분석 결과 UI를 구현한다. 실제 PR 원본 table을 은재 도메인에 만들지 않는다.
