...
```json
{
  ...
  "type": "contract",
  "level": "high",
  "reason": "Response payload no longer matches the public schema."
}
```

`type` must use `ReviewRiskType`: `security`, `logic`, `performance`, `test_gap`, `maintainability`, `contract`, `migration`, `ux`, or `other`.
