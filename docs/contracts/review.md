...
Public review read/API contract.

## Consumed By

- Dashboard/Canvas
- Code review room frontend
- Agent runtime review workflow
- GitHub/Task consumers that decorate PR/task context with review analysis
...
현재 Review controller는 `/api` prefix 없이 아래 path를 제공한다.
MVP target path는 `docs/api-contract-v1.md`의 `/api/...` route로 rebaseline한다.

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/pull-requests/:pullRequestId/review-room` | Current app-server API for creating a review room from a PR summary |
| `GET` | `/code-review-rooms/:roomId` | Current app-server API for review room details |
| `POST` | `/pull-requests/:pullRequestId/analysis` | Current app-server API for requesting PR analysis |
| `GET` | `/pull-requests/:pullRequestId/analysis` | Current app-server API for PR analysis lifecycle result |
| `GET` | `/pull-requests/:pullRequestId/analysis-summary` | Current app-server API for Dashboard/Canvas PR analysis summary |
| `GET` | `/pull-request-analyses/:analysisId/graph` | Current app-server API for review graph |
| `GET` | `/pull-request-analyses/:analysisId/canvas` | Current app-server API alias for review graph canvas |
| `PATCH` | `/review-nodes/:nodeId/state` | Current app-server API for reviewer node state |
| `POST` | `/code-review-rooms/:roomId/comments` | Current app-server API for review comments |
| `POST` | `/pull-request-analyses/:analysisId/checklist-items` | Current app-server API for checklist items |

## Deferred APIs

These routes are schema/fixture or follow-up PR scope until their app-server implementations land.
...
- `PRAnalysisSummary`, `ReviewCanvasSummary`, `ReviewNodeDetail`, `ReviewRiskSummary`의 필드 제거, 타입 변경, 의미 변경은 breaking change로 본다.
- breaking change가 필요하면 먼저 새 필드를 추가하고 기존 필드는 최소 한 PR 주기 동안 deprecated 상태로 유지한다.
- migration plan에는 affected consumers, fallback/default value, 제거 예정 시점을 포함한다.
- Dashboard, Canvas, Agent runtime, GitHub/Task consumer에게 contract PR에서 변경 내용을 공유하고 승인을 받은 뒤 구현 PR을 merge한다.
