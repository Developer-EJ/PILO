...
최신 `origin/dev`에는 Progress HTTP controller가 없다. 아래 Progress API는 아직
...
Canonical public path 후보는 `/workspaces/:workspaceId/progress/summary`다.
`/workspaces/:workspaceId/progress`는 public contract가 아니며, consumer는 사용하지
않는다. Progress controller를 구현할 때는 실제 runtime prefix 여부를 PR에서 함께
명시해야 한다.
