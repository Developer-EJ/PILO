...
|---|---|---|---|
| `GET` | `/api/workspaces/:workspaceId/progress/summary` | 현재 진행률 요약 | 동현 Dashboard, 세인 Agent, 주형 |

`GET /api/workspaces/:workspaceId/progress/summary`는 현재 Task 테이블의
삭제되지 않은 Task를 읽어 요청 시점에 `ProgressSummary`를 계산한다. Query로
`milestoneId`를 넘기면 같은 workspace의 해당 Milestone에 연결된 Task만 집계한다.
...
`/api/workspaces/:workspaceId/progress`는 public contract가 아니며, consumer는
사용하지 않는다.
