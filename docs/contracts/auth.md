...
The Auth controller is exposed through the app-server global prefix as `/api/auth/...`.
  ...
  "providers": [
    {
      ...
      "callbackUrl": "https://api.pilo.dev/api/auth/google/callback",
...
- `GET /api/auth/{provider}/start?next=/canvas`
...
로그아웃/비인증 상태는 아래처럼 표현한다. Frontend auth client가 `/api/auth/me`의 `401`을 이 형태로 변환해 사용한다.
