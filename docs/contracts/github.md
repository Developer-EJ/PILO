...
Review domain consumes this source read model to create its own `changed_files` and `changed_functions`. It is GitHub PR source metadata, not a Review analysis result.
...
`patch` can be `null` for binary files, very large files, or provider responses that omit a patch. Review must treat `pullRequestId + path + sha` as source identity when creating its own changed file/function records.
...
- Review consumes `PullRequestChangedFileSummary` for changed file/diff source metadata; Review owns analysis output, `changed_files`, and `changed_functions`.
...
- `PullRequestSummary` and `PullRequestChangedFileSummary` fields are public read model contract fields.
