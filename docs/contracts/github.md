...
GitHub App 설치 연결, 연결 상태 조회, 연결 해제, synced repository/Issue/PR read
model 조회, Task-Issue/PR 매핑을 담당한다.

provider 기반 Issue/PR 동기화, changed file 제공, webhook 처리는 public read model과
schema 후보가 있지만 현재 `dev` app-server controller에는 없다. 구현 전까지는
Current Runtime API로 취급하지 않는다.
...
| `GET` | `/api/workspaces/:workspaceId/github/repositories` | Workspace에 저장된 repository read model 조회 | 주형 GitHub UI, 동현 Dashboard |
| `GET` | `/api/repositories/:repositoryId/issues` | 저장된 Issue read model 조회 | 주형 GitHub UI, Task |
| `GET` | `/api/repositories/:repositoryId/pull-requests` | 저장된 PR read model 조회 | 주형 GitHub UI, 은재 Review |
| `POST` | `/api/tasks/:taskId/github-issues` | Task에서 local Issue read model 생성 후 Task와 연결 | Task, GitHub UI |
| `POST` | `/api/github/issues/:issueId/link-task` | 기존 Issue read model을 Task와 연결 | Task, GitHub UI |
| `POST` | `/api/tasks/:taskId/pull-requests/:pullRequestId` | Task에서 PR read model 연결 | Task, GitHub UI |
| `POST` | `/api/github/pull-requests/:pullRequestId/link-task` | PR read model에서 Task 연결 | Task, GitHub UI |
