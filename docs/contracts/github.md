...
read model과 schema 후보가 이미 있지만 최신 `origin/dev`의 app-server controller에는
아직 구현되어 있지 않다. 구현 전까지는 Provided API로 취급하지 않는다.
...
|---|---|---|---|
| `POST` | `/workspaces/:workspaceId/github/connections` | GitHub App 설치 flow 시작 | 주형, 동현 설정 UI |
| `GET` | `/workspaces/:workspaceId/github/connections` | Workspace GitHub 연결 상태 조회 | 주형, 동현 설정 UI |
| `DELETE` | `/workspaces/:workspaceId/github/connections/:connectionId` | GitHub 연결 해제 또는 revoked 처리 | 주형, 동현 설정 UI |
| `GET` | `/github/app/callback` | GitHub App 설치 callback 처리 | GitHub App redirect |

주의: 이 controller는 현재 `@Controller("api")`가 아니라 경로 자체를
`workspaces/:workspaceId/github/connections`, `github/app`으로 선언한다. 따라서
현재 runtime path에는 `/api` prefix가 없다. prefix를 통일하려면 별도 breaking
contract PR이 필요하다.

## Planned / Deferred APIs

아래 API는 문서/fixture/schema 후보이지만 최신 `origin/dev` controller에는 없다.
다른 팀은 이 API를 현재 호출하면 안 된다.

| Method | Path | Status | Notes |
|---|---|---|---|
| `GET` | `/workspaces/:workspaceId/github/repositories` | deferred | 연결 저장소 목록 조회 후속 PR 필요 |
| `POST` | `/workspaces/:workspaceId/github/repositories/sync` | deferred | provider sync 후속 PR 필요 |
| `GET` | `/repositories/:repositoryId/issues` | deferred | Issue 목록 API 후속 PR 필요 |
| `POST` | `/tasks/:taskId/github-issues` | deferred | Task에서 Issue 생성/연결 후속 PR 필요 |
| `GET` | `/repositories/:repositoryId/pull-requests` | deferred | PR 목록 API 후속 PR 필요 |
| `GET` | `/pull-requests/:pullRequestId/changed-files` | deferred | Review가 소비할 changed file source 후속 PR 필요 |
| `POST` | `/tasks/:taskId/pull-requests/:pullRequestId` | deferred | Task-PR 연결 후속 PR 필요 |
| `POST` | `/github/webhooks` | deferred | webhook 수신 후속 PR 필요 |

### Repository Sync Preconditions

`POST /workspaces/:workspaceId/github/repositories/sync`가 구현되면 workspace에는
...
`POST /workspaces/:workspaceId/github/connections`는 pending install intent를 만들고
`workspaceId`를 `state/nonce`에 묶는다. `GET /github/app/callback`은 `state/nonce`를
