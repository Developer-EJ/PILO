...
- `pilo-public-contracts.schema.json`: 핵심 read model, action payload, 공통 DTO 정의
  ...
  - 진호 Meeting/Voice 기준: `MeetingAgenda`, `MeetingReportSummary`, `MeetingDecisionSummary`, `MeetingReportRiskSummary`, `MeetingReportNextAgenda`, `MeetingReportCanvasEntityRef`, `MeetingActionItem`, `MeetingActionItemTaskDraftResponse`, `VoiceScaffoldResponse`, `VoiceRoom`, `VoiceSession`
