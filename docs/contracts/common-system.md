...
현재 `temp-dev`에서 공통 시스템 쪽 실제 HTTP API로 확인되는 것은 health check다.
...
아래 API는 공통 시스템 계약 후보이지만 현재 `temp-dev`에는 controller가 없다.
다른 도메인은 현재 runtime에서 호출하면 안 된다.

Minimal Notification is a product Target from `docs/mvp-scope-v1.md`, but it is
Deferred for current runtime. DevOps/공통 Backend gatekeeper ownership is enough
to protect table/contract changes, but not enough to declare release-blocking
runtime completion. A follow-up owner assignment and Common runtime PR are
required before Notification becomes MVP Current.
