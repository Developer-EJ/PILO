...
`NotificationCreateRequest.type` must use `NotificationType` from `docs/contracts/schemas/pilo-public-contracts.schema.json`.
...
`linkedEntityType` must use `CrossDomainEntityType`; do not invent new string values inside a feature PR. Add a new value to the owner-local schema fragment first, then serialize it through a Contract Integration PR.
