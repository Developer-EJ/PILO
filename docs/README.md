...
아래 문서가 현재 `temp-dev`를 다시 살리기 위한 기준 문서다.
...
| 7 | `docs/mvp-contract-v0.md` | 현재 `temp-dev` 구현 상태표 |
...
- Official bootstrap file name is `agent.md`. This repository does not use `AGENTS.md` as a second source of truth.
- `docs/db/pilo_erd_schema.sql` is the broad SQL baseline for local bootstrap and long-term MVP shape.
- `apps/app-server/prisma/schema.prisma` is the current Prisma-backed runtime subset. A SQL table existing in the baseline does not by itself mean the app-server HTTP API is Current Runtime.
- Current callable APIs are determined by app-server controllers plus the global `/api` prefix, then recorded in `docs/contracts/*` under `Current Runtime APIs`.
- Fixture/mock fields must stay inside `docs/contracts/schemas/pilo-public-contracts.schema.json`.
