...
- 인접 `review_order` relation은 target node가 실제로 오른쪽에 있을 때만 짧은 route를 사용한다. 같은 rank에서 위아래로 분기된 node 사이는 하단 orthogonal lane을 사용한다.
...
- 같은 rank에 배치된 인접 번호 사이의 `review_order` route는 node를 가로지르는 대각선이 아니라 하단 lane을 사용한다.
