...
전환 전 CI plan에서는 새 legacy/redirect repository variable이 아직 없을 수 있으므로
각각 `[]`, `[]`, `302`를 안전한 bootstrap 기본값으로 사용한다. 실제 cutover plan은
승인된 CLI `-var` 값으로 canonical/legacy 도메인을 고정하며, 위 repository variable은
canonical health와 callback 기능 검증이 성공한 뒤 등록한다.
...
활성 사용자가 없는 maintenance window에서 dual-callback staging 없이 각 provider의
callback을 legacy에서 canonical 값으로 직접 바꾼다. Terraform apply 전에는 provider
설정 화면의 저장 성공만 확인하고 기능 검증은 시도하지 않는다. 승인된 plan 적용,
ECS steady-state 대기, canonical/legacy HTTP semantics 확인이 끝난 뒤 Google/GitHub
login, GitHub OAuth, ProjectV2 OAuth, GitHub App setup을 하나씩 실제 검증한다.
Google authorization request의 `redirect_uri`는 등록된 authorized redirect URI와
정확히 일치해야 한다.
...
3. 현재 provider/repository 값을 기록하고 callback 5개를 canonical 값으로 하나씩
   바꾸되 provider 저장 성공만 확인한다.
4. 새 ACM 인증서, DNS aliases, CloudFront/ALB 구성과 canonical ECS origin을 apply한다.
5. 영향받는 ECS 서비스가 steady state가 될 때까지 기다린다.
6. canonical Frontend/API는 2xx, legacy API는 redirect 없는 2xx, legacy Frontend는
   exact `302`와 path/query 보존 `Location`인지 확인한다.
7. Google/GitHub login, GitHub OAuth, ProjectV2 OAuth, GitHub App setup을 하나씩
   실제 검증한 뒤 GitHub App webhook을 canonical API origin으로 바꿔 test delivery한다.
8. GitHub repository variables의 Frontend/API origin을 변경한다.
9. App Server, Realtime Server, Worker, Frontend를 순서대로 재배포하고 전체 smoke
   test를 수행한다.
