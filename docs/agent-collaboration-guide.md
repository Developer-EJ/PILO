...
- 공통 Agent Runtime과 Orchestrator는 E가 관리한다.
- A, B, C, D는 각자 도메인별 Agent Workflow를 관리한다.
...
| 담당 | 도메인 | 핵심 책임 |
|---|---|---|
| A | Workspace / Dashboard / Canvas | 프로젝트 공간, 대시보드, 시각화 |
| B | Task / GitHub / Progress | 작업 관리, GitHub 연동, 진행률 |
| C | Meeting / Voice / Report | 회의, 음성, 회의록, 액션 아이템 |
| D | Code Review Room / PR Analysis | PR 분석, 코드 리뷰, 변경 영향도 |
| E | Agent Runtime / Orchestrator / Planning | 공통 Agent 실행 구조, 계약, 프로젝트 기획 |
...
|---|---|---|---|---|---|
| A | `workspace`, `dashboard`, `canvas` | `workspace` | 필요 시 `collaboration` 조회 | 없음 | `docs/contracts/workspace.md` |
| B | `task`, `github`, `progress` | `task`, `github`, `progress` | 없음 | `task` workflow | `docs/contracts/task.md`, `github.md`, `progress.md` |
| C | `meeting`, `voice`, `report` | `meeting`, `report` | `meeting`, `voice` | `meeting` workflow | `docs/contracts/meeting.md` |
| D | `review` | `review` | 없음 | `review` workflow | `docs/contracts/review.md` |
| E | `agent`, `planning` | `agent`, `planning` | 공통 event 필요 시 | `agent`, `planning` | `docs/contracts/agent.md`, `agent-actions.md` |
...
| `apps/frontend/src/shared` | UI, hook, type 등 재사용 코드만 둔다. |
...
7. Agent Workflow는 각 도메인이 소유하되, 실행 상태, trace, action contract는 E의 Agent Runtime 규칙을 따른다.
...
```text
[A][workspace] Workspace 목록 화면 구현
[B][task] Task 생성 API 구현
[C][meeting] 회의록 생성 workflow 구현
[D][review] PR 변경 파일 분석 구현
[E][agent] Agent action contract v1 정의
```
...
## Consumed By
- A: Dashboard, Canvas
- C: Meeting action item to task draft
- D: PR to task link
- E: Agent action execution
...
- A, B, C, D가 workflow를 만들더라도 action schema의 최종 형식은 E가 관리한다.
...
```text
apps/frontend/src/domains/task/
  components/
    TaskList.tsx
    TaskStatusBadge.tsx
  hooks/
...
```text
apps/app-server/src/domains/task/
  task.module.ts
  task.controller.ts
  task.service.ts
  task.repository.ts
...
```text
apps/ai-worker/app/domains/review/
  router.py
  service.py
  schemas.py
  workflow.py
...
```text
YYYYMMDDHHMM_owner_domain_action

202607011130_B_task_create_tasks
202607021020_C_meeting_create_reports
202607031430_E_agent_create_agent_runs
```
...
| Agent action contract 변경 | E |
| GitHub, Task, Progress 변경 | B |
| Meeting, Voice, Report 변경 | C |
| PR Analysis 변경 | D |
| Workspace, Dashboard, Canvas 변경 | A |
...
- PR 분석 구현 중 GitHub OAuth 로직을 수정한다.
