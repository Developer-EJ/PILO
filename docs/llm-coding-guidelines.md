...
- 정당한 사유가 있다면 사용자의 요청에 반대 의견을 제시한다.
- 불분명한 부분이 있다면 작업을 중단하고, 혼란스러운 부분을 구체적으로 질문한다.
...
- "유효성 검사 추가"는 "잘못된 입력에 대한 테스트 작성 후 통과 확인"으로 바꾼다.
- "버그 수정"은 "버그를 재현하는 테스트 작성 후 통과 확인"으로 바꾼다.
- "X 리팩토링"은 "리팩토링 전후의 테스트 통과 확인"으로 바꾼다.
