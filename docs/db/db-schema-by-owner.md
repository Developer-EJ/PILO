...
| `oauth_accounts`        | `user_id`, `provider`, `provider_user_id`, `provider_email`, `scopes`, `token_type`, `access_token_encrypted`, `refresh_token_encrypted`, `token_expires_at` | `users 1:N oauth_accounts`. Google/GitHub 로그인 계정                                              |
| `auth_sessions`         | `user_id`, `refresh_token_hash`, `token_hash_algorithm`, `secret_version`, `user_agent`, `ip_address`, `expires_at`, `revoked_at`                            | `users 1:N auth_sessions`. 로그인 세션                                                             |
...
| `voice_sessions`                | `voice_room_id`, `meeting_id`, `member_id`, `recording_status`, `started_at`, `ended_at`                                   | 멤버별 음성 세션/녹음 상태          |
...
| `pull_request_analyses`  | `pull_request_id`, `purpose_summary`, `impact_summary`, `test_recommendation`, `risk_level`, `analysis_status`, `ok_count`, `discuss_count`, `risk_count`, `conclusion`, `error_trace` | PR 분석 결과 루트         |
| `review_graphs`          | `analysis_id`, `pull_request_id`, `summary`, `intent_summary`, `review_strategy`, `review_order`                                                                         | PR 분석 그래프            |
...
| `review_nodes`           | `id`, `graph_id`, `node_type`, `label`, `file_path`, `function_name`, `risk_level`, `review_order`, `role_summary`, `review_reason`, `position`                          | 리뷰 그래프 노드          |
