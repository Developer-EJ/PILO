...
계약이다. schema 생성은 mutation이고 테이블 집중 보기는 read-only view다.

### Schema 생성

AI Worker는 임의 DDL 문자열 대신 `SqlErdSchemaSpecV1`을 만들고,
...
schema 생성 tool 이름은 `generate_sql_erd`이며 `riskLevel=medium`,
...
`focus_sql_erd_tables`는 `riskLevel=low`, `executionMode=auto`인 read-only
tool이다. 현재 SQLtoERD session의 source, model, layout을 변경하지 않고 Activity Log도 남기지 않는다.
공개 `inspect_sql_erd_schema` tool과 session 후보 선택 계약은 hard cutover로 제거됐다.

`focus_sql_erd_tables` input:

```ts
type FocusSqlErdTablesInput = {
  featureQuery: string; // trim/공백 정리 후 1~200자
};
```

- `surface=sql_erd` request context와 현재 `sessionId`가 반드시 필요하다. Planner는 session ID,
  revision, model fingerprint, table ref를 입력에 넣지 않는다. SQLtoERD 화면 밖에서 다른 session을
  검색하거나 schema projection만 조회하는 Agent 요청은 지원하지 않는다.
...
```ts
type SqlErdTableFocusResourceRef = {
  domain: "sqltoerd";
  resourceType: "session";
  resourceId: string;
  label: string;
