...
`inspect_sql_erd_schema`와 `focus_sql_erd_tables`는 모두 `riskLevel=low`,
`executionMode=auto`인 read-only tool이다. session, source, model, layout을 변경하지 않고
Activity Log도 남기지 않는다.

`inspect_sql_erd_schema` input:
...
- App Server는 최신 session을 다시 조회해 `sessionRevision` 일치, compact ref 존재·중복·역할
  겹침, 각 related와 primary 사이의 직접 FK를 검증한다. stale revision은 `409 CONFLICT`다.
