...
`planning_started_at`부터 120초를 넘기면 현재
...
Frontend
  -> App Server Agent API
    ...
    -> 즉시 publisher wake / 15초 recovery sweep / 120초 planning deadline
...
- `contextRef`는 현재 run과 같은 thread·Workspace·요청 사용자의 최근 완료 tool step에서만 해소한다.
  App Server는 해소된 Meeting resource를 현재 사용자 권한으로 다시 검증한다. 0개, 여러 개, 다른 type,
...
| 값 | 의미 |
| --- | --- |
| `planning` | run이 생성됐고 AI Worker가 요청을 해석하거나 tool plan을 만드는 중(최대 120초) |
...
`planning_started_at`를 점검하며 120초를 넘긴 turn만 `AGENT_PLANNING_TIMEOUT`으로 실패 처리한다.
따라서 terminal 전이는 turn 시작 후 최대 약 135초(120초 deadline + 다음 15초 sweep) 안에 일어난다. 이 처리에는 pending/running planner step
정리와 privacy-safe `planning_timeout` log가 포함되며, 이전 turn의 지연 SQS message는 새 turn을
terminalize할 수 없다. Frontend는 run 전체가 아니라 연속된 `planning` 구간만 190초까지 polling하며,
...
- publisher는 1, 2, 4, 8, 16분 backoff로 최대 5회 재시도하지만, 그 재시도는 120초 planning deadline을
...
- AI Worker planner의 infrastructure failure는 원본 SQS queue에서 세 번째 수신까지 재시도한다. Agent 전용
  SQS visibility는 90초이며, 재시도 중에도 App Server의 120초 turn deadline이 run과 남은 planner step을 safe
...
- `search_meeting_transcript`는 `query`와 선택 MeetingReport selector를 받는다. raw `reportId`는
  ...
  않고, 권한 있는 namespaced source ID만 grounded-answer 경로에 전달한다.
