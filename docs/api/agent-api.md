...
- thread는 UI 목록이나 “새 대화” 버튼으로 선택하지 않는다. 첫 요청에서 만들고 서버 `last_activity_at`
  기준 1시간 안의 다음 요청을 자동으로 같은 thread에 연결한다. 1시간 후에는 새 thread를 자동 생성한다.
  Frontend는 `threadId`를 저장하거나 request에 주입하지 않는다. 새 의도로 대기 run을 종료하면
  `last_activity_at`과 관계없이 종료한 run의 서버 소유 `thread_id`를 replacement run에 그대로 사용한다.
...
| `cancelled` | confirmation 거절·만료, 명시적 사용자 취소 또는 같은 thread의 새 의도 전환으로 실행이 종료됨 |
...
클라이언트는 `threadId`를 보내거나 선택할 수 없다. 서버는 `activeRunId`가 현재 Workspace와 요청 사용자에
...
| `new_intent` | 기존 대기 run을 `cancelled`로 끝내고 같은 `thread_id`의 replacement run을 같은 transaction에서 생성 |
...
새 outbox 생성, 두 log 저장을 원자적으로 수행한다. 기존 run에는
...
| `started_new` | `new_intent` | 새 run | 취소된 기존 run 또는 `null` | replacement 또는 일반 신규 run 생성 |
