...
### 내부 NextToolDecision 계약

- Router가 선택한 capability chain과 현재 사용자 입력 이후의 completed Tool state로
  `agent-next-tool-decision:v1`을 결정적으로 계산한다.
- `outputSummary.nextToolDecision`은 `status`, `candidateCount`, `reasonCode`, 선택 후보가 하나일 때의
  `boundToolName`, confirmation 여부, completed Tool 수만 기록한다. raw Tool input/output과 resource ID는
  기록하지 않는다. Resolver target domain과 target/constraint canonical fingerprint는
  `contextTargetDomain`, `contextStateSha256`로 남기며 opaque reference 원문은 남기지 않는다.
- 후보가 하나이면 Planner는 Tool 이름을 선택하지 않고 input과 사용자 메시지만 생성하며, Worker가
  `boundToolName`을 바인딩한다. 여러 후보이면 해당 frontier만 Planner에 제공한다.
- Worker와 App Server는 chain 밖 Tool, prerequisite 미충족, 완료 Tool 재호출, context surface 불일치를
  거부한다. confirmation 승인은 실제 Tool 실행 전에 같은 검증을 다시 수행한다.
- 같은 사용자 prompt cycle의 후속 planning turn은 직전 routed capability를 재사용한다. 새 사용자 입력은
  이전 Tool 완료 상태를 새 chain의 prerequisite로 사용하지 않고 다시 routing한다.

이 계약은 내부 planner/handoff 동작만 변경하며 public endpoint, request/response, DB schema는 변경하지
않는다.
...
- thread context는 최근 완료 run 6개, UTF-8 12 KiB, Meeting resource reference 12개로 제한한다.
...
- `contextRef`는 현재 run과 같은 thread·Workspace·요청 사용자의 최근 완료 tool step에서만 해소한다.
  App Server는 해소된 resource를 현재 사용자 권한으로 다시 검증한다. 0개, 여러 개, 다른 type,
  stale·expired·consumed reference는 실행이나 confirmation을 만들지 않고 clarification으로 종료한다.
- Worker는 Router보다 먼저 후속 발화의 지시어, 순번, 정정·제외, 조건 보완과 명시적 domain 전환을
  `ContextResolution`으로 정규화한다. 정확히 하나의 opaque reference만 결정된 `resolved` 결과만
  Router/Planner에 전달한다. 후보가 없거나 여러 개이거나 stale·충돌이면 `needs_clarification`으로
  전환하고 Router, Planner, Tool, confirmation을 호출하지 않는다. context 지시가 없으면
  `not_applicable`로 처리한다.
- App Server는 `resolved` target의 실제 resource를 Tool 실행 직전에 다시 해소한다. planner가 다른
  `contextRef`, 실제 resource ID 또는 raw provider identifier를 제출하거나 해소 결과의 domain/type이
  다르면 안전한 진단 코드와 함께 clarification으로 전환한다. write Tool은 이 검증 뒤 기존 confirmation
  계약을 따르며, 승인 시점에도 저장된 target과 권한·상태를 다시 검증한다.
- Meeting action item write는 `actionItemContextRef` 하나 또는 `reportContextRef`와 1-based `ordinal`을
  ...
  실행한다. 새 `agent-tools:v7` planner output은 같은 raw ID를 제출해도 adapter를 사용할 수 없다.
...
- thread는 UI 목록이나 “새 대화” 버튼으로 선택하지 않는다. 첫 요청에서 만들고 서버 `last_activity_at`
  ...
  Frontend는 Workspace별 마지막 run ID만 `sessionStorage`에 저장해 reload 후 기존 권한 검증 endpoint로
  상태를 복구하며 `threadId`를 저장하거나 request에 주입하지 않는다.
...
- `requestContext.surface`가 `canvas`, `sql_erd`, `pr_review`이면 App Server의 planning snapshot과 실행
  registry, AI Worker의 모든 retrieval mode가 tool/capability/domain을 각각 `canvas`, `sql_erd`,
  `pr_review` 하나로 제한한다. surface와 다른 domain이나 capability는 planner와 실행 단계로 전달하지 않는다.
...
- App Server가 새로 발행하는 capability catalog는 `agent-tool-capabilities:v3`이다. capability마다
  positive 변형과 negation/exclusion/correction/anaphora/domain-switch boundary, selector kind,
  prerequisite/follow-up/terminal, read/write, execution mode, confirmation, supported/unsupported 상태를
  포함한다. Worker는 SHA와 registry schema digest를 검증하고 누락·중복·알 수 없는 selector·chain 또는
  confirmation 충돌을 fail-closed한다. 이 catalog에는 raw resource ID를 넣지 않는다.
...
| `resultSets` | array | `domain`, `resourceType`, `contextRef`, `label`, `ordinal`, `generation`, `source`, 선택적 `status`만 포함 |
...
Worker가 `resolved`를 반환한 경우 planner step의 `outputSummary.contextResolution`에는 다음 bounded
projection이 저장된다. 이 값은 실제 resource ID나 권한 증명이 아니며 App Server 재검증 입력이다.

| Field | Type | 설명 |
| --- | --- | --- |
| `version` | `agent-context-resolution:v1` | resolution contract version |
| `status` | `resolved` \| `needs_clarification` \| `not_applicable` | 해소 결과 |
| `reasonCode` | string | 내부 식별자를 포함하지 않는 bounded 진단 코드 |
| `target` | object \| undefined | `resolved`일 때 `contextRef`, `domain`, `resourceType`, `ordinal`, `generation`, `source` |
| `constraints` | object | 상태·제외 reference 등 정규화된 bounded 조건 |
| `clarificationQuestion` | string \| null | clarification 때 사용자에게 표시할 안전한 질문 |

candidate `source`의 reference는 같은 run과 thread에서 선택 예약됐고 아직 후속 Tool에 적용되지 않은
미만료 candidate만 허용한다. 후속 Tool이 완료된 뒤 재사용하려는 candidate는 stale로 처리한다. tool result
`source`의 reference는 bounded thread context에 남아 있는 최근 완료 결과만 허용한다.
