...
- claim된 worker는 delivery의 remote installation과 ProjectV2 node에 일치하는 모든 선택 workspace target을 다시 조회한다. One GitHub GraphQL target-item fetch is fanned out to all matching selected workspace targets under the one delivery lease; `processed`는 모든 target reconcile 또는 archive가 성공한 뒤에만 기록한다. 선택 target이 남아 있지 않으면 GraphQL 없이 delivery를 `processed`로 완료한다.
