...
- Board hydration reconciles remote columns to the current Status option snapshot exactly.
  Status option columns absent from the fetched snapshot are deleted by `status_option_github_id`,
  even after `status_option_id` has been nulled by the FK `ON DELETE SET NULL`. Retained Status
  option columns are ordered by the GitHub option order, and exactly one local `Unmapped` column
  remains after the Status options (position `0` when there are no Status options).
- Items that still reference a deleted Status option move to `Unmapped` because issue refresh runs
  after stale remote columns are deleted.
...
- GitHub Integration의 `project_v2_fields`와 `full` sync field 단계에서는 field/option current snapshot 반영과 기존 Board hydration이 같은 DB transaction 안에서 완료된 뒤 Board invalidation을 발행한다.
