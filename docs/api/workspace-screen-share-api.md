...
- 모든 endpoint는 `WorkspaceService.assertWorkspaceAccess`로 현재 사용자의 Workspace
  membership을 다시 확인한다.
- 성공 응답은 `{ "success": true, "data": ... }` envelope를 사용한다.
- LiveKit API key, secret, provider raw error는 응답이나 로그에 노출하지 않는다.
- LiveKit 입장 token의 만료 시간은 발급 시점 기준 1시간이다.
...
- 다른 사용자의 `starting` 또는 `active` reservation도
  `409 SCREEN_SHARE_ALREADY_ACTIVE`다.
...
- reservation과 종료는 각각 하나의 Lua script로 두 key를 원자적으로 변경한다.
...
상태를 종료하고 ended event를 발행한다. 철회 event와 API/token 발급이 경쟁하면
membership을 다시 확인해 발급이나 join을 취소한다. Redis event가 지연되더라도
모든 조회와 token 발급 endpoint는 매 요청 membership을 확인한다.

## 저장 범위

- 화면 공유 session, LiveKit room reverse lookup, realtime delivery state는 Redis의
  12시간 TTL 범위에만 존재한다.
