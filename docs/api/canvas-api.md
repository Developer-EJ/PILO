...
| Canvas 종류 | 목록·생성 | metadata·shape 조회 | shape·viewport mutation | Canvas Agent |
| --- | --- | --- | --- | --- |
| `freeform` | 허용 | 허용 | 허용 | 허용 |
| active `review` room 연결 Canvas | 제외 | 허용 | 허용 | 제외 |
| completed `review` room 연결 Canvas | 제외 | 허용 | 거부 | 제외 |
| room 미연결 `review` Canvas | 제외 | 거부 | 거부 | 제외 |
