...
응답은 `Shape Payload`와 같은 full shape data이며 `contentHash`와 `revision`을
포함한다. 이 endpoint는 기존 호출자와 단일 shape 복구 경로의 호환성을 위해 유지한다.

현재 Classic Canvas의 viewport와 Frame 자식 조회도 `rawShape`를 포함한 full shape
snapshot을 반환하고, checkpoint 전 최신 상태는 roomState patch로 전달한다. 따라서
Classic Canvas 편집기는 shape 클릭만으로 상세 조회를 다시 호출하지 않는다. 클릭 시
DB의 이전 checkpoint snapshot이 최신 roomState를 덮어쓰지 않도록 viewport/Frame
Lazy Loading과 roomState snapshot을 그대로 사용한다.
...
- 프론트 viewport/Frame 자식 조회는 `@tanstack/react-query` query key,
  cancellation, cache invalidation과 local dirty shape 방어를 함께 사용한다.
