...
같은 room file은 기존 위치·크기·parent·표시 순서를 유지한다. 즉, 저장된 geometry 또는 pin된
node는 최초 materialization이나 새 revision 분석으로 덮어쓰지 않으며, 저장된 file node가 없는
최초 Canvas에만 위 자동 배치를 적용한다. 이후 새 file node는 기존 deterministic grid 초기
위치를 받는다. 현재 버전에서 사라진
...
semantic relation은 rank를 바꾸지 않는다. 자동 정렬은 사용자가 실행한 경우에만 현재 Flow의
이동 가능한 node 위치를 저장하며, pin된 node와 이미 저장된 geometry는 보존한다. Canvas를
불러오는 것만으로 기존 좌표를 재배치하지 않는다.
