...
**Phase 전환 gate:** `0-1`부터 `0-5`까지의 구현은 완료됐다. 남은 `0-6` 자동 quality gate가 완료되기
전에는 **Phase 1B, Phase 2 이후 실행 E2E 확대, Phase 4의 model/retrieval rollout**으로 전환하지 않는다.
DB 비의존적인 Phase 1A fixture·평가 보강은 provisional 작업으로 병렬 진행할 수 있으나, model 교체나
production 판단 근거로 사용하지 않는다.
