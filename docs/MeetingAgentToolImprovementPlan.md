...
- [ ] **G. dev 개방 gate를 판정한다.** 아래 최소 조건을 모두 만족하면 dev의 모든 Workspace를 팀원에게
  개방한다.
  - [ ] dev 배포에서 planner routing flag를 `shortlist`/`shadow`로 전환하고, Agent Worker task rollout 뒤
...
- [ ] **7. 계약·검증 결과를 기록한다.** Agent/Meeting API 문서 영향 여부를 확인하고, build/lint/format,
...
- [x] **4-F / #1475. Prompt Injection Security Gate를 구현한다.**
  - 범위: 사용자 발화, bounded Meeting thread resource와 grounded evidence를 untrusted data로 취급한다.
    ...
    retrieval·planner 전에 차단하되 부정형 보안 결정·회고·인용 문장은 인접 반례로 허용한다. system
...
- [ ] **4-E. Meeting dev 공개 gate와 smoke test를 완료한다.**
  - 선행: 4-A~4-D, 4-F.
