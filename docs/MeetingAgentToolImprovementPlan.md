...
- [ ] **A. runtime 계약과 flag를 고정한다.** App Server가 eligible schema snapshot·catalog SHA·request
  ...
  기록한다. planner routing flag는 dev에서만 기본 off로 두고 Workspace 또는 allowlist 단위로 켤 수 있게
  하며, 요청 단위 mode와 catalog/snapshot SHA를 run/step에 남긴다.
- [ ] **B. Meeting capability routing을 연결한다.** Meeting read capability와 current-meeting control
  ...
  않은 채 안전한 clarification 또는 legacy planner fallback으로 끝낸다.
...
- [ ] **E. feature flag fallback과 rollback을 검증한다.** flag off는 기존 all-eligible planner와 동일한
  ...
  또는 legacy fallback을 선택한다. flag를 즉시 off하면 새 배포 없이 legacy 경로로 되돌아가는 integration
  test와 dev runbook을 추가한다.
...
- [ ] **G. dev 개방 gate를 판정한다.** 아래 최소 조건을 모두 만족하면 allowlist를 팀원에게 개방한다.
  ...
  - [ ] retrieval·selector·candidate 오류는 write 없이 clarification 또는 legacy fallback으로 끝난다.
