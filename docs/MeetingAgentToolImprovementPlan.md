...
> 상태: Phase 0의 DB 비의존 Tool Discovery·평가 기반과 CI quality gate를 완료했다. 개발 DB와
> migration이 준비된 현재는 **Phase 1B의 stateful planner·resolver 통합 검증**을 다음 작업으로 진행한다.
> Phase 1~3의 Meeting resolver·candidate selection·thread memory 기반을 수용 기준까지 연결한 뒤,
> Phase 4에서 이를 전 도메인이 재사용하는 Tool 검색·의도/selector·multi-turn·평가 기반으로 확장한다.
>
...
- [x] **0B / #1409 / #1399** — compact catalog SQS 전달, Worker schema digest binding,
  ...
  #1399를 완료로 닫았다.
- [x] **0-1 / `4c8bced2`** — registry inventory SHA, legacy full-tool schema/token baseline과
  fixture 누락 목록을 `dev`에 고정했다.
- [x] **0-3 / #1413** — 전 도메인 descriptor·발화 fixture, unsupported capability 경계와
  Worker의 v2 strict validator를 `dev`에 병합했다.
- [x] **0-4 / #1409 / #1413** — hard eligibility 안에서 deterministic metadata retrieval과
  semantic rerank adapter, topK shortlist, shortlist 밖 tool 거부, low-confidence legacy fallback과
  unsupported/mutation 안전 경계를 구현했다.
- [x] **0-5 / `ac43ebb8`** — domain/capability/필수 chain recall, paired legacy-shadow 비교,
  provider token usage와 privacy-safe bounded event 구현을 완료했다. AI Worker 256개 테스트는 통과했으며
  `dev` 병합은 후속 PR로 남아 있다.

**Phase 전환 gate:** `0-1`부터 `0-6`까지의 DB 비의존 기반과 CI quality gate가 완료됐다. 따라서
**Phase 1B, Phase 2 이후 실행 E2E 확대, Phase 4의 model/retrieval rollout 준비**로 전환할 수 있다.
다만 실제 provider model 교체와 production rollout은 별도 baseline·비용·지연시간·안전 gate를 통과한 뒤
판단하며, 이 Phase의 deterministic 결과만으로 production 품질을 확정하지 않는다.
...
`delegate_canvas_agent`, `recommend_pr_review_focus`는 legacy all-tool snapshot의 누락 목록으로 계속
명시하되, 0-3의 v2 catalog descriptor/fixture에서는 추적한다. 실제 provider planner의 결과 baseline은
Phase 1B의 dev 실행으로 별도 기록한다.
...
- [x] 초기 `AGENT_TOOL_RETRIEVAL_MODE=read_only_shortlist`에서 planner에는 read-only shortlist schema만
  ...
  selection helper로 이 fallback을 재현한다. 기본 mode는 `shadow`다. 이 초기 mode는 Phase 4-A에서 최종
  `shadow|shortlist` 계약으로 교체한다.
...
#### 진행 현황과 다음 실행 순서 — Phase 0

1. [x] **0-1 inventory와 legacy baseline을 자동화한다.** registry에서 현재 등록 tool을 추출해
...
3. [x] **0-5 평가·shadow·privacy-safe 관측을 완결한다.** 동일 고정 입력의 legacy/shadow 비교,
   domain/capability/필수 chain recall, 인접 intent·unsupported 오판, latency/token usage와 bounded
   telemetry를 offline evaluator에서 재현한다.
...
`tool_retrieval_quality_gate_v1.json`, full registry snapshot SHA 대조 및 App CI baseline artifact로
완료됐다.
...
>
> **현재 우선순위: Phase 1B.** DB migration과 개발 DB가 준비됐으므로, 실제 OpenAI dev baseline,
> resolver의 cross-workspace 실행, stateful planner/App Server 통합 평가를 수행한다. 이 결과는
> production rollout 자체가 아니라 이후 Phase 3 E2E와 model 비교의 수용 기준으로 사용한다.
...
AI Worker 로컬 전체 검증은 `black --check app tests`, `ruff check app tests`, `python -m pytest`
(264 passed), `python -m compileall app`까지 통과했다. 다만 실제 OpenAI dev baseline, stateful
...
다음 구현 순서: catalog/schema·구조 검증 → canonical fixture → held-out fixture → UUID 회귀 → local
unit/script 검증까지 완료했다. 이제 **1) context follow-up·인접 intent의 stateful planner 실행 평가,
2) 0/1/N·동명이인·cross-workspace resolver 실행 fixture, 3) 고정 기준일·모델의 OpenAI dev baseline과
오인식 분류 기록** 순서로 #1371을 완료한다. 세 항목은 같은 suite/catalog SHA를 사용하고, 하나의
Phase 1B PR/Issue 체크리스트에서 함께 추적한다.
...
- [x] 사용자 목표를 stable capability catalog로 정의하고, 하나의 capability가 조건부 tool chain에,
...
- [x] 사용자 발화와 현재 surface·thread context로 domain/action 후보를 먼저 좁히고 관련 tool schema만
...
- [x] **2. capability catalog와 Tool descriptor 계약을 분리한다.** capability는 사용자가 달성하려는
...
- [x] **3. 2단계 Tool 검색 계층을 구현한다.** App Server는 기존처럼 surface·context의 hard eligibility를
  ...
  tool만 포함한다.
- [x] **4. planner handoff를 shortlist 기반으로 바꾼다.** v1 transport는 호환성을 위해 eligible schema
  ...
  shortlist 밖 tool name·field·UUID는 normalization에서 거부한다. dev shortlist runtime의 catalog·retrieval
  장애나 낮은 confidence는 legacy planner를 자동 호출하지 않고 clarification으로 종료하며, shadow/offline
  비교만 legacy all-eligible planner를 사용한다. 어느 경로에서도 supported write를 거짓 `unsupported`로
  만들지 않고 App Server validation/confirmation을 유지한다. 후속 최적화에서 schema fetch 경계를 추가해
  SQS payload 자체도 줄인다.
...
- [ ] **14. domain adapter를 순차 개방하고 E2E를 완료한다.** catalog-only와 retriever shadow로 동작
  변화 없이 관측한 뒤 `shortlist`에서 read/write capability와 필요한 prerequisite chain을 함께
  적용한다. Meeting에서 자연어 → Tool 검색 → selector → 후보/clarification → original goal 재개 →
  tool/confirmation을 검증하고 Calendar, Board, Canvas·SQLtoERD·PR Review, Drive 순으로 같은 gate를
  적용한다. write capability는 각 domain의 confirmation gate 통과 뒤 활성화하며 UI·run/step·server-side
  context·SQS readback과 suite/catalog/tool snapshot SHA를 기록한다.
...
- [x] **A. runtime 계약과 flag를 고정한다.** App Server가 eligible schema snapshot·catalog SHA·request
  context를 Worker job에 전달하고, Worker는 `legacy`, `shadow`, `shortlist` routing mode를 명시적으로
  기록한다. planner routing flag는 dev의 모든 Workspace에서 기본 on으로 두고, ECS 환경 변수
  `AGENT_TOOL_RETRIEVAL_MODE`를 `shadow`로 변경한 뒤 Agent Worker task rollout으로 legacy 경로로
  되돌린다. 요청 단위 mode와 catalog/snapshot SHA를 run/step에 남긴다.
- [x] **B. Meeting capability routing을 연결한다.** Meeting read capability와 current-meeting control
...
- [x] **C. selector와 Phase 3 재개를 planner 경로에 연결한다.** lookup 결과가 0건·복수·만료·stale이면
...
- [x] **D. dev 관측 event를 완성한다.** raw 발화·raw payload·resource reference 없이 routing mode,
...
- [x] **E. feature flag fallback과 rollback을 검증한다.** flag off는 기존 all-eligible planner와 동일한
  결과를 유지하고, flag on에서 retriever·catalog 오류가 나면 write를 실행하지 않고 clarification
  으로 끝낸다. `AGENT_TOOL_RETRIEVAL_MODE=shadow` 환경 변수와 Agent Worker task rollout으로 legacy
  경로를 복구하는 integration test와 dev runbook을 추가한다.
- [x] **F. Meeting dev E2E와 eval gate를 추가한다.** (1) 최신/상대 날짜 회의록 lookup, (2) 복수 후보
...
- [x] **G. dev 개방 gate를 판정한다.** 아래 최소 조건을 모두 만족하면 dev의 모든 Workspace를 팀원에게
  개방한다.
  - [x] dev 배포에서 planner routing flag를 `shortlist`/`shadow`로 전환하고, Agent Worker task rollout 뒤
    legacy planner를 복구할 수 있다.
  - [x] Meeting lookup → 후보 선택 → original goal 재개가 동일 run/thread에서 동작한다.
  - [x] retrieval·selector·candidate 오류는 write 없이 clarification으로 끝난다.
  - [x] run/step 또는 운영 event에서 tool 선택과 fallback/실패 taxonomy를 확인할 수 있다.
  - [x] canonical·held-out·multi-turn regression, privacy 검증, flag-on/flag-off integration test가 통과한다.

#### Phase 4 Meeting 회의록 selector·상대 날짜 구현 체크리스트 — #1453

> 범위: Phase 4의 첫 후속 vertical로 Meeting 회의록 조회 selector를 완결한다. 공개 Meeting API를 새로
> 만들지 않고 Agent 내부 resolver와 기존 Meeting 조회 경계를 사용한다. 공개 API 계약 또는 DB schema가
> 필요해지면 Meeting·DB Schema owner 확인과 별도 계약 변경으로 분리한다.

- [x] **1. 조회 기본값과 limit 계약을 고정한다.** 기간과 개수를 모두 생략한 회의록 요청은
  `createdAt DESC` 최신 1건을 선택한다. 개수가 명시되면 같은 정렬에서 정확히 해당 개수만 반환하며,
  Agent 전용 조회의 limit은 1..100 범위를 그대로 보존한다.
- [x] **2. 상대 날짜 selector를 Workspace timezone으로 정규화한다.** 지난주는 직전 월요일 00:00부터
  이번 월요일 00:00 전까지, 다음 주는 다음 월요일 00:00부터 그 다음 월요일 00:00 전까지로 해석한다.
  최근과 “며칠 전”은 현재 시점 기준 최근 7일로, “최근 N건”은 기간보다 count와 최신 정렬을 우선한다.
- [x] **3. 다가오는 주말 경계를 명시한다.** 주말은 Workspace timezone의 다가오는 토요일 00:00부터
  월요일 00:00 전까지로 해석한다. 토·일에 요청한 “다가오는 주말”은 현재 진행 중인 주말이 아니라
  다음 주 토요일 00:00부터 월요일 00:00 전까지로 해석하고 regression으로 고정한다.
- [x] **4. tool schema·resolver·service 경계를 연결한다.** `list_meeting_reports`,
  `get_meeting_report`, `summarize_meeting_report`의 selector와 planner-facing schema를 동기화한다.
  기존 Meeting service의 공개 API limit 정책을 넓히지 않고 Agent 전용 resolver에서 단건·명시 count
  계약을 보장한다.
- [x] **5. clarification과 multi-turn 안전 경계를 고정한다.** 0건·복수 후보·검증 불가 selector는
  UUID나 내부 오류를 노출하지 않는 clarification으로 끝내며, 후보 선택 뒤에는 이미 완료한 lookup을
  반복하지 않고 original goal을 재개한다.
- [x] **6. catalog·eval·회귀를 추가한다.** 최신 1건, 최신 N건, 지난주, 다음 주, 최근, 며칠 전,
  다가오는 주말과 인접 반례를 capability example·retrieval fixture·App Server execution/resource
  resolver·AI Worker multi-turn regression에 고정한다.
- [x] **7. 계약·검증 결과를 기록한다.** Agent/Meeting API 문서 영향 여부를 확인하고, build/lint/format,
  App Server Agent script, AI Worker Ruff/Black/pytest/quality gate, dev smoke flow 결과를 이슈와 PR에
  남긴다.
...
- [x] **dev fallback·catalog integrity 정책** — 낮은 confidence·retriever 오류·schema budget 초과와
  catalog 누락은 기존 전체-tool shadow 경로로 fallback한다. catalog SHA 불일치와 eligible schema/catalog
  digest 불일치는 변조·계약 손상으로 보고 planner를 호출하지 않는 clarification으로 fail-closed 처리한다.
  어느 경우에도 mutation을 임의 실행하거나 supported 요청을 `unsupported`로 단정하지 않으며, write의
  confirmation·권한·멱등성·실행 직전 재검증을 우회하지 않는다.
...
- [x] **dev flag 노출 범위** — dev의 모든 Workspace에서 planner routing을 기본 활성화한다. ECS 환경 변수
  `AGENT_TOOL_RETRIEVAL_MODE`를 dev feature flag로 사용하며 production 활성화는 별도 rollout gate로
  판단한다.
...
- [x] **Meeting 회의록 기본 선택** — 기간과 개수를 모두 생략한 “최근 회의록”은 `createdAt DESC` 기준
  최신 1건을 반환한다. 개수를 명시하면 기간 대신 그 개수와 최신 정렬을 우선한다. “최근 7일”처럼
  기간을 명시한 경우에만 날짜 범위를 적용한다.
- [x] **Meeting 상대 날짜 기본값** — “지난주”와 “다음 주”는 월요일~일요일 기준의 직전/다음 주로
  해석한다. “최근”과 “며칠 전”은 최근 7일, “주말”은 다가오는 주말로 해석한다. 토·일의
  “다가오는 주말”은 다음 주말을 뜻한다. 경계를 정할 수 없는 날짜 없는 “그때”는 clarification을
  요청한다.
...
> 아래 작업 단위는 **1개 이슈 = 1개 브랜치 = 1개 PR**로 진행한다. 이미 완료한 공통 retrieval
> foundation은 다시 구현하지 않고, dev runtime과 Meeting 첫 vertical을 완료한 뒤 도메인 adapter를
> 하나씩 연다.

- [x] **4-A / #1459. Retrieval runtime·flag·trace를 dev 배포 경계에 연결한다.**
  - 범위: 최종 `AGENT_TOOL_RETRIEVAL_MODE=shadow|shortlist` 계약으로 정리한다. `shortlist`는 read/write
    capability를 모두 후보로 삼고, 선택된 write capability의 prerequisite tool chain을 예산 안에서 함께
    포함한다. catalog/eligible snapshot/shortlist SHA, retrieval mode·fallback reason을 run/step의 안전한
    projection에 기록하고 ECS dev 설정·rollback 절차를 준비한다.
  - 제외: remote flag, Workspace allowlist, 자연어 selector, provider model 교체.
  - 완료 기준: `shadow`는 기존 full-tool 실행 결과를 유지한다. `shortlist`는 read/write shortlist와 필요한
    prerequisite chain만 planner에 제공하고 shortlist 밖 tool 출력은 거부한다. 낮은 confidence·retriever
    오류·schema budget 초과와 catalog 누락은 전체-tool shadow 경로로 fallback한다. catalog SHA 불일치와
    eligible schema/catalog digest 불일치는 planner를 호출하지 않는 clarification으로 처리한다. 이 어떤
    경로도 write의 confirmation·권한·멱등성·실행 직전 재검증을 건너뛰지 않는다.

- [x] **4-B / #1453. Meeting 자연어 read selector를 구조화한다.**
  - 범위: Meeting의 `dateRange`, `limit`, `sort`, `roomSelector`, `reportSelector`를 planner contract와
    resolver에 연결한다. “최근”, 상대 날짜, 수량 요청의 기본값과 모호한 날짜 clarification을 구현한다.
    `sort`는 사용자 입력 selector로 노출하지 않고 모든 회의록 목록 조회에서 `createdAt DESC`로 고정한다.
  - 제외: 다른 도메인 adapter, 후보 선택 뒤 thread 재개, write capability 확대.
  - 완료 기준: “최근 회의록”, “최근 3건”, 단일/복수 날짜·회의방 조회가 raw UUID 없이 안전한 결과 또는
    clarification으로 끝난다.

- [x] **4-C / #1465. #1355 bounded thread context를 완료한다.**
  - 범위: safe `contextRef`, 최신성·정렬·byte 제한, Workspace/user 재검증과 “그 회의록”, “2번 작업”의
    단일·모호 context 해소를 #1355 범위에서 구현한다.
  - 제외: retrieval 자체 변경, 다른 도메인 selector 확장.
  - 완료 기준: 단일 context는 재사용하고 복수·만료·다른 thread context는 clarification으로 끝나며,
    raw prompt/payload/UUID/token은 planner context에 포함되지 않는다.

- [x] **4-D / #1473. Meeting 후보 선택 뒤 original goal 재개를 연결한다.**
  - 선행: Phase 3, 4-B, 4-C.
  - 범위: candidate selection 뒤 원래 capability·미해결 selector slot·safe `contextRef`를 사용해,
    같은 list/read를 반복하지 않고 상세·요약·confirmation의 다음 tool로 진행한다.
  - 확정 계약: 현재 후보 선택을 만드는 모든 Meeting read/write 흐름에 적용한다. 여러 selector가
    모호하면 한 번에 하나만 server-owned 후보로 선택하고 다음 selector clarification을 이어간다.
    선택 row가 가리키는 직전 clarification step과 retrieval의 terminal goal을 사용하며, 정상 선택 뒤
    완료된 lookup은 다시 호출하지 않는다.
  - 완료 기준: 0/N·만료·stale·다른 user/Workspace/run/thread 후보는 write/confirmation 없이
    clarification으로 끝난다. 정상 선택은 원래 요청을 한 번만 재개하며, 이후 완료 tool step이 생기면
    해당 resume state를 무효화한다. write는 다음 selector까지 모두 해소된 뒤에만 기존 confirmation·권한·
    실행 직전 재검증으로 진입한다.

- [x] **4-F / #1475. Prompt Injection Security Gate를 구현한다.**
  - 범위: 최초 사용자 발화와 현재 `turnSequence`를 재개한 최신 follow-up 한 건, bounded Meeting thread
    resource와 grounded evidence를 untrusted data로 취급한다. 과거 user message 전체를 다시 검사해 안전한
    재시도를 막지 않는다.
    completed tool result와 선택 후보 label/description도 context 생성 시 구조화된 source kind로 분리한다.
    명시적 system instruction override, 민감정보 추출, shortlist·flag·권한·confirmation 우회 신호는
    retrieval·planner 전에 차단하되 부정형 보안 결정·회고·인용 문장은 인접 반례로 허용한다. 부정된
    clause 뒤에 같은 우회 명령을 다시 지시하면 새 actionable signal로 차단한다. system
    policy·registry·Workspace 권한·server-owned context와 App Server 실행 검증만 제어 영역으로 유지한다.
  - 관측: raw 공격 문자열을 저장하지 않고 detector version, `prompt_injection_suspected`, source kind,
    bounded signal taxonomy와 count만 planner step에 기록한다. grounded answer도 injection evidence를
    provider에 보내지 않는다.
  - 검증: 사용자·Meeting content·tool-result fixture와 정상 보안 논의 인접 반례를 deterministic CI gate로
    고정한다. `shadow`·`shortlist` 모두 full-tool fallback, planner, execution handoff, write confirmation
    후보를 만들지 않으며 기존 App Server confirmation·권한·멱등성·실행 직전 재검증을 유지한다.

- [ ] **4-E / #1477. Meeting dev 공개 gate와 smoke test를 완료한다.**
  - 선행: 4-A~4-D, 4-F.
  - 범위: Meeting canonical/held-out/negative/multi-turn eval, privacy/confirmation/권한/멱등성/실행 직전
    재검증 regression, `shadow`·`shortlist` dev smoke test, 관측 확인과 ECS rollback runbook을
    자동화·문서화한다. Meeting write E2E로 회의 나가기, 녹음 종료, action item 수정·승인을 검증한다.
  - 완료 기준: read/write Meeting shortlist를 dev에 공개할 수 있고, retrieval/planner 실패는 안전한
    clarification 또는 전체-tool `shadow` fallback으로 처리된다. shortlist 여부와 무관하게 모든 write는
    confirmation·권한·멱등성·실행 직전 재검증을 통과하며, rollback과 관측 확인을 재현할 수 있다.
  - [x] 구현: App CI는 registry-bound deterministic preflight와 실제 Meeting write runtime suite를 실행한다.
    provider credential이 필요한 공개 승인 workflow는 registry에서 평가 input을 만들고 canonical 216건·held-out
    54건·negative 72건·multi-turn 54건을 `shadow`·`shortlist`로 각각 실제 평가한다. fixture 개수만으로
    `passed=true`를 만들지 않는다.
  - [x] 구현: dev smoke는 ECS 안정 상태와 task definition mode뿐 아니라 실제 Agent API read run 완료,
    planner `toolRetrieval` 관측, write run의 `waiting_confirmation`, 승인 전 mutation 부재를 검사한다. smoke가
    만든 confirmation은 승인하지 않고 reject해 정리한다.
  - [ ] 실행: `Evaluate Agent Planner`의 네 variant × 두 mode threshold와 `Phase 4-E Agent Dev Smoke`의
    `shortlist` 결과가 실제 dev 배포에서 통과해야 4-E를 완료 처리한다. rollback rehearsal은 Terraform으로
    `shadow`를 배포한 뒤 같은 smoke를 `expected_mode=shadow`로 다시 통과시킨다.
...
- [x] AI Worker의 server-side repository read가 thread context를 정렬·sanitize·byte 제한해 planner
  handoff의 최소 projection만 만들고, App Server가 resource reference를 현재 Workspace·사용자 기준으로
  재검증한다.
- [x] AI Worker planner가 bounded context에서 “그 회의”, “방금 회의록”, “2번 후속 작업”을
  selector로 해소한다. 후보가 불명확하면 `needs_clarification`으로 끝내며 write candidate를
  만들지 않는다.
- [x] Frontend Agent widget은 `threadId`를 선택하거나 생성하지 않고 서버가 복구한 현재 thread의
  run 상태만 표시한다. 새로고침 뒤 1시간 이내 thread를 자동 복구하고, 1시간 경과 뒤 다음 요청에서
  서버가 새 thread를 만드는 정책을 검증한다. “새 대화” 버튼과 thread 목록은 추가하지 않는다.
- [x] 동일 thread의 동시 전송·retry, thread 간 isolation, cross-user/workspace 차단, bounded
...
- [x] **1. 현재 계약과 회귀 기준 고정** — `074_create_meeting_agent_workflow.sql`의
...
- [x] **4. Run 생성·idempotency 연결** — `POST /agent/runs`는 server-owned thread에만 새 run을
  ...
  thread 선택은 서버가 수행한다.
- [x] **5. Bounded context projection** — 새 run을 planning할 때 AI Worker repository가 같은 thread의
  최근 terminal run만 newest-first로 제한해 읽고, planner 순서에 맞춰 oldest-first projection으로
...
- [x] **6. Opaque resource reference** — planner에는 UUID 대신 thread 내 `contextRef`를 보낸다.
...
- [x] **8. Meeting planner policy** — “그 회의록”, “방금 보여준 2번 작업”, “그 회의 나가줘”는
...
- [x] **9. Frontend thread lifecycle** — Agent widget은 server-owned thread를 request에 주입하지 않는다.
...
- [x] **11. App Server 회귀** — thread 생성/재사용, clientRequestId conflict, ownership/404 은닉,
  context byte·turn budget, expired cleanup, opaque ref 검증, pending confirmation과 context write
  재검증을 script test로 추가한다.
- [x] **12. AI Worker 회귀** — internal context projection sanitizer, stale outbox generation,
