...
- [x] **dev 낮은 confidence·retrieval 오류 정책** — bounded domain 확장 뒤에도 shortlist recall을
  보장할 수 없거나 retriever·catalog 오류가 나면, dev 실행 경로는 legacy planner를 자동 호출하지
  않고 clarification으로 종료한다. ECS 환경 변수 변경과 Agent Worker task rollout으로 legacy 경로를
  복구할 수 있으며, 어느 경우에도 mutation을 임의 실행하거나 supported 요청을 `unsupported`로 단정하지
  않는다.
