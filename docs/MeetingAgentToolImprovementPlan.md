...
- [x] **4-E / #1477. Meeting dev 공개 gate와 smoke test를 완료한다.**
  - 선행: 4-A~4-D, 4-F.
  ...
  - 구현: App CI가 registry-bound Meeting write 계약, retrieval/security artifact, canonical 216건·held-out
    54건·negative 72건·multi-turn 54건, dev `shortlist` 기본값과 `shadow` runbook을 privacy-safe readiness
    artifact로 결합한다. 별도 수동 workflow는 실제 ECS service 안정 상태와 task definition mode를 read-only로
    확인하며 mode 변경과 resource mutation은 수행하지 않는다.
