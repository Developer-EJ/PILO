...
- [x] **낮은 confidence fallback** — 관련 domain bundle을 bounded 확장하고, 그래도 recall이 보장되지
  않으면 기존 all-eligible planner로 fallback한다. 어느 경우에도 mutation을 임의 실행하지 않으며 기존
  App Server confirmation·권한·상태 재검증을 그대로 적용한다. 낮은 confidence 자체가 곧바로 supported
  요청의 `unsupported` 판정이 되지 않게 한다.
...
- [ ] **Meeting 상대 날짜 기본값** — 추천: Workspace timezone의 최근 **7개 완료된 calendar day**를
  `from` inclusive, 오늘 시작 시각을 `to` exclusive로 사용한다. “최근 3건”처럼 수량이 있으면 기간
  대신 limit/정렬을 우선한다.
- [ ] **모호한 상대 날짜 정책** — 추천: “지난주”, “다음 주”는 ISO week(월요일 시작)로 해석하고,
