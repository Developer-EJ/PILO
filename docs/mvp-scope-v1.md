...
| Task type | `development`, `planning`, `meeting`, `review`, `document`, `bug`, `etc` |
| 완료 조건 | acceptance criteria 목록 |
...
| Memo node | Canvas 전용 텍스트 메모 |
| Reference node | Task, Report, Issue, PR 참조 노드 |
| Node position | 위치, 크기 저장 |
| Edge | 노드 간 기본 연결 |
| Node detail panel | 원본 요약과 원본 화면 이동 |
