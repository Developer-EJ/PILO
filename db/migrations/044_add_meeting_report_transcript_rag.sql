...
COMMENT ON TABLE public.meeting_report_transcript_embedding_jobs IS
  'Durable internal indexing jobs for MeetingReport raw transcript RAG. Public clients must not access this table.';

COMMENT ON TABLE public.meeting_report_transcript_chunks IS
  'Bounded raw transcript chunks and pgvector embeddings. Retrieval must join MeetingReport and Meeting to enforce Workspace access and match transcript_hash to the current raw transcript.';
